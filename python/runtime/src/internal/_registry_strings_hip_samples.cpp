...
// Kernel registry partition for the `hip_samples` bucket. Compiles
...
const char* k_hip_navatala_samples_scatter_add_float32 = R"kernel(
...
const char* k_hip_navatala_samples_float32_add = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_samples_triangle_normals2 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_samples_axpy_fallback = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_samples_det_prepare_scatter_0 = R"kernel(
#include <hip/hip_runtime.h>
...
const KernelArgumentInfo kAbiArgs_hip_navatala_samples_scatter_add_float32[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_samples_scatter_add_float32 = {
  1,
  "navatala_samples_scatter_add_float32",
  "hip",
  "navatala_samples_scatter_add_float32",
  "kernel:hip:navatala_samples_scatter_add_float32",
  "abi-r1:hip:navatala_samples_scatter_add_float32",
  "abi-r1:hip:navatala_samples_scatter_add_float32",
  3,
  kAbiArgs_hip_navatala_samples_scatter_add_float32
};

const KernelArgumentInfo kAbiArgs_hip_navatala_samples_float32_add[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_samples_float32_add = {
  1,
  "navatala_samples_float32_add",
  "hip",
  "navatala_samples_float32_add",
  "kernel:hip:navatala_samples_float32_add",
  "abi-r1:hip:navatala_samples_float32_add",
  "abi-r1:hip:navatala_samples_float32_add",
  3,
  kAbiArgs_hip_navatala_samples_float32_add
};

const KernelArgumentInfo kAbiArgs_hip_navatala_samples_triangle_normals2[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_samples_triangle_normals2 = {
  1,
  "navatala_samples_triangle_normals2",
  "hip",
  "navatala_samples_triangle_normals2",
  "kernel:hip:navatala_samples_triangle_normals2",
  "abi-r1:hip:navatala_samples_triangle_normals2",
  "abi-r1:hip:navatala_samples_triangle_normals2",
  3,
  kAbiArgs_hip_navatala_samples_triangle_normals2
};

const KernelArgumentInfo kAbiArgs_hip_navatala_samples_axpy_fallback[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_samples_axpy_fallback = {
  1,
  "navatala_samples_axpy_fallback",
  "hip",
  "navatala_samples_axpy_fallback",
  "kernel:hip:navatala_samples_axpy_fallback",
  "abi-r1:hip:navatala_samples_axpy_fallback",
  "abi-r1:hip:navatala_samples_axpy_fallback",
  4,
  kAbiArgs_hip_navatala_samples_axpy_fallback
};

const KernelArgumentInfo kAbiArgs_hip_navatala_samples_det_prepare_scatter_0[] = {
  { "dstIdx", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "messages", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_samples_det_prepare_scatter_0 = {
  1,
  "navatala_samples_det_prepare_scatter_0",
  "hip",
  "navatala_samples_det_prepare_scatter_0",
  "kernel:hip:navatala_samples_det_prepare_scatter_0",
  "abi-r1:hip:navatala_samples_det_prepare_scatter_0",
  "abi-r1:hip:navatala_samples_det_prepare_scatter_0",
  5,
  kAbiArgs_hip_navatala_samples_det_prepare_scatter_0
};

bool tryGetKernelAbiManifest_hip_samples(const std::string& backend, const std::string& kernelName, const KernelAbiManifestInfo*& out) {
  if (backend == "hip" && kernelName == "navatala_samples_scatter_add_float32") {
    out = &kAbiManifest_hip_navatala_samples_scatter_add_float32;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_samples_float32_add") {
    out = &kAbiManifest_hip_navatala_samples_float32_add;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_samples_triangle_normals2") {
    out = &kAbiManifest_hip_navatala_samples_triangle_normals2;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_samples_axpy_fallback") {
    out = &kAbiManifest_hip_navatala_samples_axpy_fallback;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_samples_det_prepare_scatter_0") {
    out = &kAbiManifest_hip_navatala_samples_det_prepare_scatter_0;
    return true;
  }
  out = nullptr;
  return false;
...
bool tryGetKernelSource_hip_samples(const std::string& backend, const std::string& kernelName, GpuRuntime::ProgramSource& out) {
  if (backend == "hip" && kernelName == "navatala_samples_scatter_add_float32") {
    out.kind = GpuRuntime::ProgramSource::Kind::HipCpp;
    out.entryPoint = "navatala_samples_scatter_add_float32";
    std::string_view sv(k_hip_navatala_samples_scatter_add_float32);
  ...
  if (backend == "hip" && kernelName == "navatala_samples_float32_add") {
    ...
    std::string_view sv(k_hip_navatala_samples_float32_add);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_samples_triangle_normals2") {
    ...
    std::string_view sv(k_hip_navatala_samples_triangle_normals2);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_samples_axpy_fallback") {
    ...
    std::string_view sv(k_hip_navatala_samples_axpy_fallback);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_samples_det_prepare_scatter_0") {
    ...
    std::string_view sv(k_hip_navatala_samples_det_prepare_scatter_0);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  return false;
