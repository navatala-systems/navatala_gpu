...
// Kernel registry partition for the `hip_cfd` bucket. Compiles
...
const char* k_hip_navatala_cfd_boundary_force_partials = R"kernel(
...
const char* k_hip_navatala_cfd_alpha_proc_face_interp = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_alpha_proc_face_interp(const int* procFaces, const float* sendOwn, const float* recvNei, const int* counts, float* alphaF) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  const int nSafeMax = (((int)(counts[0])) > 0 ? ((int)(counts[0])) - 1 : 0);
  const int safeIdx = (gid0 < nSafeMax ? gid0 : nSafeMax);
  if (gid0 >= ((int)(counts[0]))) return;
  if (((int)((int)(blockIdx.x * blockDim.x + threadIdx.x))) >= counts[0]) {
    ...
    float a = (__uint_as_float(0x3f000000u) * (sendOwn[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))] + recvNei[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))]));
    float clamped = a;
    if (clamped < __uint_as_float(0x00000000u)) {
      clamped = __uint_as_float(0x00000000u);
    }
    if (clamped > __uint_as_float(0x3f800000u)) {
      clamped = __uint_as_float(0x3f800000u);
    }
    alphaF[face] = clamped;
  }
}
...
const char* k_hip_navatala_cfd_mules_proc_face_update = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_mules_proc_face_update(const int* procFaces, const int* counts, const float* phiCorr, float* lambda, const float* lambdapCell, const float* lambdamCell, const int* ownerAll, const float* nbrLambdaP, const float* nbrLambdaM) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  const int nSafeMax = (((int)(counts[0])) > 0 ? ((int)(counts[0])) - 1 : 0);
  const int safeIdx = (gid0 < nSafeMax ? gid0 : nSafeMax);
  if (gid0 >= ((int)(counts[0]))) return;
  if (((int)((int)(blockIdx.x * blockDim.x + threadIdx.x))) >= counts[0]) {
    ...
    float other = nbrLambdaP[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
    if (lim > other) {
      lim = other;
    }
    if (pc > __uint_as_float(0x00000000u)) {
      float limPos = lambdapCell[o];
      float otherPos = nbrLambdaM[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
      if (limPos > otherPos) {
        limPos = otherPos;
      }
      lim = limPos;
...
const char* k_hip_navatala_cfd_mules_proc_face_update_f64 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_pack_owner_values = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_pack_owner_values_f64 = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_pack_owner_values_f64(const double* x, const int* owner, const int* procFaces, const int* counts, double* outSend) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  const int nSafeMax = (((int)(counts[0])) > 0 ? ((int)(counts[0])) - 1 : 0);
  const int safeIdx = (gid0 < nSafeMax ? gid0 : nSafeMax);
  if (gid0 >= ((int)(counts[0]))) return;
...
const char* k_hip_navatala_cfd_scatter_values_and_mask = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_pack_mu_grad_owner = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_pack_owner_rho_vec3 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_pack_owner_vec3 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_pack_owner_vec3_mu = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_scatter_mu_grad_ghost = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_scatter_rho_vec3_and_mask = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_scatter_vec3_and_mask = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_scatter_vec3_mu_and_mask = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_div_rho_phi_u_explicit = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_hby_a_from_u_and_terms = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_apply_sp_to_hby_a = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_laplacian_scalar_explicit = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_laplacian_vector_explicit = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_predictor_hby_a_from_source = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_predictor_inverse_diag = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_predictor_phi_hby_a_fuse = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_axpy_cells = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_ddt_diagonal = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_ddt_source = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_ddt_source(const float* oldField, const float* vol0, const int* counts, const float* paramsF, float* outSource) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  const int nSafeMax = (((int)(counts[0])) > 0 ? ((int)(counts[0])) - 1 : 0);
  const int safeIdx = (gid0 < nSafeMax ? gid0 : nSafeMax);
  if (gid0 >= ((int)(counts[0]))) return;
  if (((int)((int)(blockIdx.x * blockDim.x + threadIdx.x))) >= counts[0]) {
    ...
    float oldVal = oldField[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
    float volVal = vol0[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
    float src = ((oldVal * volVal) * paramsF[0]);
    outSource[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))] = src;
  }
}

)kernel";
const char* k_hip_navatala_cfd_div_upwind_coeffs = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_fvm_sp = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_fvm_su_sp = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_laplacian_coeffs = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_laplacian_coeffs_accum = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_linear_upwind_correction_source = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_matrix_relax = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_neg_sum_diag = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_pack_boundary_adjacent_cells = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_scalar_jacobi_clamp_min = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_scalar_jacobi_update = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_scalar_ldu_coupled_interface_add = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_scalar_ldu_coupled_interface_add(const int* faceCells, const float* coeffs, const float* recvNeighbourX, const unsigned int* counts, float* ax) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  const int nSafeMax = (((int)(counts[0])) > 0 ? ((int)(counts[0])) - 1 : 0);
  const int safeIdx = (gid0 < nSafeMax ? gid0 : nSafeMax);
  if (gid0 >= ((int)(counts[0]))) return;
...
const char* k_hip_navatala_cfd_scalar_ldu_mat_vec = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_scalar_ldu_multi_color_gs_sweep = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_sum_mag_off_diag = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_upwind_weights = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_upwind_weights(const float* flux, const int* counts, float* outWeights) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  ...
  if (((int)((int)(blockIdx.x * blockDim.x + threadIdx.x))) >= counts[2]) {
    ...
    float phi = flux[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
    float w = __uint_as_float(0x00000000u);
    if (phi >= __uint_as_float(0x00000000u)) {
      w = __uint_as_float(0x3f800000u);
    } else {
      w = __uint_as_float(0x00000000u);
    }
    outWeights[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))] = w;
  }
}

)kernel";
const char* k_hip_navatala_cfd_gamg_axpy_in_place = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_gamg_compute_scale_factor = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_gamg_dot_finalize = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_gamg_negate_scalar = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_gamg_scale_correction = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mg_dic_build = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mg_dic_smooth = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mg_coarse_jacobi = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mg_coarse_matvec = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mg_coarse_rhs = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mg_find_best_match = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mg_pair_match = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mg_prolong_add = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mg_renumber_aggregates = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mg_renumber_propagate = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mg_resolve_chains = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mg_restrict_sum = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mg_strength_of_connection = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mg_zero_coarse = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_phi_correct = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_phi_correct_boundary = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_phi_correct_internal = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_axpy_in_place = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_bc_dirichlet_face_flux = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_bc_sn_grad_face_flux = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_diag_from_cf = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_dic_apply = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_dic_build_reciprocal_d = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_dot_partials = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_face_flux = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_inv_diag = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_laplacian_from_face_flux = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mul_by_vol_in_place = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_ref_add_ax = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_ref_add_ax_native_f64 = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_ref_add_ax_native_f64(const double* diag, const double* x, double* outAx, const int* refCell) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int c = refCell[0];
  if (c < 0) {
    return;
...
const char* k_hip_navatala_cfd_ref_add_b = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_ref_add_b_native_f64 = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_ref_add_b_native_f64(const double* diag, double* b, const int* refCell, const double* refVal) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int c = refCell[0];
  if (c < 0) {
    return;
...
const char* k_hip_navatala_cfd_rhs_sub = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_shift_in_place = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_sum_abs_partials = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_upper_from_cf = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_xpay_in_place = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_dot_finalize_partials = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_negate_scalar = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_pcg_compute_alpha = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_pcg_compute_beta = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_exact_mat_vec = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_exact_mat_vec_f64 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_exact_mat_vec_native_f64 = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_exact_mat_vec_native_f64(const double* x, const int* owner, const int* neighbour, const unsigned int* offsets, const unsigned int* faceIdx, const int* sign, const double* upper, const double* lower, const double* diag, const int* counts, double* outAx) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  const int nSafeMax = (((int)(counts[0])) > 0 ? ((int)(counts[0])) - 1 : 0);
  const int safeIdx = (gid0 < nSafeMax ? gid0 : nSafeMax);
  if (gid0 >= ((int)(counts[0]))) return;
  if ((int)(blockIdx.x * blockDim.x + threadIdx.x) >= ((int)(counts[0]))) {
    ...
    double s = (diag[(int)(blockIdx.x * blockDim.x + threadIdx.x)] * x[(int)(blockIdx.x * blockDim.x + threadIdx.x)]);
    unsigned int beg = offsets[(int)(blockIdx.x * blockDim.x + threadIdx.x)];
    int c1 = ((int)(blockIdx.x * blockDim.x + threadIdx.x) + 1);
    unsigned int end = offsets[c1];
    int len = ((int)((end - beg)));
...
const char* k_hip_navatala_cfd_face_flux_boundary = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_face_flux_internal = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_pressure_face_flux_correction_corrected = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_pressure_sn_grad = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_u_correct = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_u_correct_reconstruct = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_grad_alpha_cell = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_grad_alpha_cell_f64 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_n_hatf_all = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_n_hatf_all_f64 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_stf_all_faces = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_stf_all_faces_f64 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_k_omega_s_s_t_blending = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_k_omega_s_s_t_nut_compute = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_k_omega_s_s_t_sources = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mu_eff_from_rho_nu_eff = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_mu_eff_from_rho_nut_and_nu_mix = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_turbulence_add_explicit_source_device = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_turbulence_dirichlet_cell_source = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_turbulence_dirichlet_face_elimination = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_two_mag_sqr_symm_grad_u = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_attention_row_softmax = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_attention_value_projection = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_gather_mean = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_layer_norm_forward = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_radius_count = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_scaled_dot_product = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_scatter_add_atomic = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_spectral_complex_mul = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_spectral_conv1d_mode_sliced = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_spectral_conv2d_mode_sliced = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_spectral_conv3d_mode_sliced = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_spectral_conv4d_mode_sliced = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_compute_face_area_pair_weights_f32 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_restrict_by_agglomeration_f32 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_restrict_by_agglomeration_gather_f32 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_prolongate_by_agglomeration_f32 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_diag_inv_apply_f32 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_csr_sp_m_v_mixed_f32_f64 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_chebyshev_sweep_mixed_f32_f64 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_ell_sp_m_v_mixed_f32_f64 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_chebyshev_smooth_post_fused_mixed = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_gamg_proc_pack_f32 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_gamg_proc_add_f32 = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_gamg_proc_add_gather_f32 = R"kernel(
#include <hip/hip_runtime.h>
...
const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_boundary_force_partials[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_boundary_force_partials = {
  1,
  "navatala_cfd_boundary_force_partials",
  "hip",
  "navatala_cfd_boundary_force_partials",
  "kernel:hip:navatala_cfd_boundary_force_partials",
  "abi-r1:hip:navatala_cfd_boundary_force_partials",
  "abi-r1:hip:navatala_cfd_boundary_force_partials",
  4,
  kAbiArgs_hip_navatala_cfd_boundary_force_partials
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_alpha_proc_face_interp[] = {
  { "procFaces", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "sendOwn", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "recvNei", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 4, 4, 256, nullptr, 0, 0 },
  { "alphaF", 4, KernelArgumentRole::InputOutput, KernelAccessMode::ReadWrite, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_alpha_proc_face_interp = {
  1,
  "navatala_cfd_alpha_proc_face_interp",
  "hip",
  "navatala_cfd_alpha_proc_face_interp",
  "kernel:hip:navatala_cfd_alpha_proc_face_interp",
  "abi-r1:hip:navatala_cfd_alpha_proc_face_interp",
  "abi-r1:hip:navatala_cfd_alpha_proc_face_interp",
  5,
  kAbiArgs_hip_navatala_cfd_alpha_proc_face_interp
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mules_proc_face_update[] = {
  { "procFaces", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  ...
  { "lambda", 3, KernelArgumentRole::InputOutput, KernelAccessMode::ReadWrite, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "lambdapCell", 4, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "lambdamCell", 5, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "ownerAll", 6, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "nbrLambdaP", 7, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "nbrLambdaM", 8, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mules_proc_face_update = {
  1,
  "navatala_cfd_mules_proc_face_update",
  "hip",
  "navatala_cfd_mules_proc_face_update",
  "kernel:hip:navatala_cfd_mules_proc_face_update",
  "abi-r1:hip:navatala_cfd_mules_proc_face_update",
  "abi-r1:hip:navatala_cfd_mules_proc_face_update",
  9,
  kAbiArgs_hip_navatala_cfd_mules_proc_face_update
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mules_proc_face_update_f64[] = {
  { "procFaces", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 4, 4, 256, nullptr, 0, 0 },
  { "phiCorr", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "lambda", 3, KernelArgumentRole::InputOutput, KernelAccessMode::ReadWrite, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mules_proc_face_update_f64 = {
  1,
  "navatala_cfd_mules_proc_face_update_f64",
  "hip",
  "navatala_cfd_mules_proc_face_update_f64",
  "kernel:hip:navatala_cfd_mules_proc_face_update_f64",
  "abi-r1:hip:navatala_cfd_mules_proc_face_update_f64",
  "abi-r1:hip:navatala_cfd_mules_proc_face_update_f64",
  9,
  kAbiArgs_hip_navatala_cfd_mules_proc_face_update_f64
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_pack_owner_values[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_pack_owner_values = {
  1,
  "navatala_cfd_pack_owner_values",
  "hip",
  "navatala_cfd_pack_owner_values",
  "kernel:hip:navatala_cfd_pack_owner_values",
  "abi-r1:hip:navatala_cfd_pack_owner_values",
  "abi-r1:hip:navatala_cfd_pack_owner_values",
  5,
  kAbiArgs_hip_navatala_cfd_pack_owner_values
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_pack_owner_values_f64[] = {
  { "x", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "owner", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "procFaces", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 4, 4, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_pack_owner_values_f64 = {
  1,
  "navatala_cfd_pack_owner_values_f64",
  "hip",
  "navatala_cfd_pack_owner_values_f64",
  "kernel:hip:navatala_cfd_pack_owner_values_f64",
  "abi-r1:hip:navatala_cfd_pack_owner_values_f64",
  "abi-r1:hip:navatala_cfd_pack_owner_values_f64",
  5,
  kAbiArgs_hip_navatala_cfd_pack_owner_values_f64
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_scatter_values_and_mask[] = {
  { "procFaces", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_scatter_values_and_mask = {
  1,
  "navatala_cfd_scatter_values_and_mask",
  "hip",
  "navatala_cfd_scatter_values_and_mask",
  "kernel:hip:navatala_cfd_scatter_values_and_mask",
  "abi-r1:hip:navatala_cfd_scatter_values_and_mask",
  "abi-r1:hip:navatala_cfd_scatter_values_and_mask",
  5,
  kAbiArgs_hip_navatala_cfd_scatter_values_and_mask
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_pack_mu_grad_owner[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_pack_mu_grad_owner = {
  1,
  "navatala_cfd_pack_mu_grad_owner",
  "hip",
  "navatala_cfd_pack_mu_grad_owner",
  "kernel:hip:navatala_cfd_pack_mu_grad_owner",
  "abi-r1:hip:navatala_cfd_pack_mu_grad_owner",
  "abi-r1:hip:navatala_cfd_pack_mu_grad_owner",
  14,
  kAbiArgs_hip_navatala_cfd_pack_mu_grad_owner
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_pack_owner_rho_vec3[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_pack_owner_rho_vec3 = {
  1,
  "navatala_cfd_pack_owner_rho_vec3",
  "hip",
  "navatala_cfd_pack_owner_rho_vec3",
  "kernel:hip:navatala_cfd_pack_owner_rho_vec3",
  "abi-r1:hip:navatala_cfd_pack_owner_rho_vec3",
  "abi-r1:hip:navatala_cfd_pack_owner_rho_vec3",
  8,
  kAbiArgs_hip_navatala_cfd_pack_owner_rho_vec3
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_pack_owner_vec3[] = {
  { "x", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_pack_owner_vec3 = {
  1,
  "navatala_cfd_pack_owner_vec3",
  "hip",
  "navatala_cfd_pack_owner_vec3",
  "kernel:hip:navatala_cfd_pack_owner_vec3",
  "abi-r1:hip:navatala_cfd_pack_owner_vec3",
  "abi-r1:hip:navatala_cfd_pack_owner_vec3",
  7,
  kAbiArgs_hip_navatala_cfd_pack_owner_vec3
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_pack_owner_vec3_mu[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_pack_owner_vec3_mu = {
  1,
  "navatala_cfd_pack_owner_vec3_mu",
  "hip",
  "navatala_cfd_pack_owner_vec3_mu",
  "kernel:hip:navatala_cfd_pack_owner_vec3_mu",
  "abi-r1:hip:navatala_cfd_pack_owner_vec3_mu",
  "abi-r1:hip:navatala_cfd_pack_owner_vec3_mu",
  8,
  kAbiArgs_hip_navatala_cfd_pack_owner_vec3_mu
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_scatter_mu_grad_ghost[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_scatter_mu_grad_ghost = {
  1,
  "navatala_cfd_scatter_mu_grad_ghost",
  "hip",
  "navatala_cfd_scatter_mu_grad_ghost",
  "kernel:hip:navatala_cfd_scatter_mu_grad_ghost",
  "abi-r1:hip:navatala_cfd_scatter_mu_grad_ghost",
  "abi-r1:hip:navatala_cfd_scatter_mu_grad_ghost",
  12,
  kAbiArgs_hip_navatala_cfd_scatter_mu_grad_ghost
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_scatter_rho_vec3_and_mask[] = {
  { "procFaces", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_scatter_rho_vec3_and_mask = {
  1,
  "navatala_cfd_scatter_rho_vec3_and_mask",
  "hip",
  "navatala_cfd_scatter_rho_vec3_and_mask",
  "kernel:hip:navatala_cfd_scatter_rho_vec3_and_mask",
  "abi-r1:hip:navatala_cfd_scatter_rho_vec3_and_mask",
  "abi-r1:hip:navatala_cfd_scatter_rho_vec3_and_mask",
  9,
  kAbiArgs_hip_navatala_cfd_scatter_rho_vec3_and_mask
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_scatter_vec3_and_mask[] = {
  { "procFaces", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "recvVals", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_scatter_vec3_and_mask = {
  1,
  "navatala_cfd_scatter_vec3_and_mask",
  "hip",
  "navatala_cfd_scatter_vec3_and_mask",
  "kernel:hip:navatala_cfd_scatter_vec3_and_mask",
  "abi-r1:hip:navatala_cfd_scatter_vec3_and_mask",
  "abi-r1:hip:navatala_cfd_scatter_vec3_and_mask",
  7,
  kAbiArgs_hip_navatala_cfd_scatter_vec3_and_mask
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_scatter_vec3_mu_and_mask[] = {
  { "procFaces", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "recvVals", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "bcX", 3, KernelArgumentRole::InputOutput, KernelAccessMode::ReadWrite, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_scatter_vec3_mu_and_mask = {
  1,
  "navatala_cfd_scatter_vec3_mu_and_mask",
  "hip",
  "navatala_cfd_scatter_vec3_mu_and_mask",
  "kernel:hip:navatala_cfd_scatter_vec3_mu_and_mask",
  "abi-r1:hip:navatala_cfd_scatter_vec3_mu_and_mask",
  "abi-r1:hip:navatala_cfd_scatter_vec3_mu_and_mask",
  8,
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_add_vol_vector = {
  1,
  "navatala_cfd_add_vol_vector",
  "hip",
  "navatala_cfd_add_vol_vector",
  "kernel:hip:navatala_cfd_add_vol_vector",
  "abi-r1:hip:navatala_cfd_add_vol_vector",
  "abi-r1:hip:navatala_cfd_add_vol_vector",
  10,
  kAbiArgs_hip_navatala_cfd_add_vol_vector
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_div_rho_phi_u_explicit[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_div_rho_phi_u_explicit = {
  1,
  "navatala_cfd_div_rho_phi_u_explicit",
  "hip",
  "navatala_cfd_div_rho_phi_u_explicit",
  "kernel:hip:navatala_cfd_div_rho_phi_u_explicit",
  "abi-r1:hip:navatala_cfd_div_rho_phi_u_explicit",
  "abi-r1:hip:navatala_cfd_div_rho_phi_u_explicit",
  22,
  kAbiArgs_hip_navatala_cfd_div_rho_phi_u_explicit
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_hby_a_from_u_and_terms[] = {
  { "ux", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "uy", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "uz", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_hby_a_from_u_and_terms = {
  1,
  "navatala_cfd_hby_a_from_u_and_terms",
  "hip",
  "navatala_cfd_hby_a_from_u_and_terms",
  "kernel:hip:navatala_cfd_hby_a_from_u_and_terms",
  "abi-r1:hip:navatala_cfd_hby_a_from_u_and_terms",
  "abi-r1:hip:navatala_cfd_hby_a_from_u_and_terms",
  18,
  kAbiArgs_hip_navatala_cfd_hby_a_from_u_and_terms
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_apply_sp_to_hby_a[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_apply_sp_to_hby_a = {
  1,
  "navatala_cfd_apply_sp_to_hby_a",
  "hip",
  "navatala_cfd_apply_sp_to_hby_a",
  "kernel:hip:navatala_cfd_apply_sp_to_hby_a",
  "abi-r1:hip:navatala_cfd_apply_sp_to_hby_a",
  "abi-r1:hip:navatala_cfd_apply_sp_to_hby_a",
  9,
  kAbiArgs_hip_navatala_cfd_apply_sp_to_hby_a
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_laplacian_scalar_explicit[] = {
  { "muCell", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_laplacian_scalar_explicit = {
  1,
  "navatala_cfd_laplacian_scalar_explicit",
  "hip",
  "navatala_cfd_laplacian_scalar_explicit",
  "kernel:hip:navatala_cfd_laplacian_scalar_explicit",
  "abi-r1:hip:navatala_cfd_laplacian_scalar_explicit",
  "abi-r1:hip:navatala_cfd_laplacian_scalar_explicit",
  17,
  kAbiArgs_hip_navatala_cfd_laplacian_scalar_explicit
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_laplacian_vector_explicit[] = {
  { "muCell", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_laplacian_vector_explicit = {
  1,
  "navatala_cfd_laplacian_vector_explicit",
  "hip",
  "navatala_cfd_laplacian_vector_explicit",
  "kernel:hip:navatala_cfd_laplacian_vector_explicit",
  "abi-r1:hip:navatala_cfd_laplacian_vector_explicit",
  "abi-r1:hip:navatala_cfd_laplacian_vector_explicit",
  28,
  kAbiArgs_hip_navatala_cfd_laplacian_vector_explicit
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_predictor_hby_a_from_source[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_predictor_hby_a_from_source = {
  1,
  "navatala_cfd_predictor_hby_a_from_source",
  "hip",
  "navatala_cfd_predictor_hby_a_from_source",
  "kernel:hip:navatala_cfd_predictor_hby_a_from_source",
  "abi-r1:hip:navatala_cfd_predictor_hby_a_from_source",
  "abi-r1:hip:navatala_cfd_predictor_hby_a_from_source",
  8,
  kAbiArgs_hip_navatala_cfd_predictor_hby_a_from_source
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_predictor_inverse_diag[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_predictor_inverse_diag = {
  1,
  "navatala_cfd_predictor_inverse_diag",
  "hip",
  "navatala_cfd_predictor_inverse_diag",
  "kernel:hip:navatala_cfd_predictor_inverse_diag",
  "abi-r1:hip:navatala_cfd_predictor_inverse_diag",
  "abi-r1:hip:navatala_cfd_predictor_inverse_diag",
  3,
  kAbiArgs_hip_navatala_cfd_predictor_inverse_diag
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_predictor_phi_hby_a_fuse[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_predictor_phi_hby_a_fuse = {
  1,
  "navatala_cfd_predictor_phi_hby_a_fuse",
  "hip",
  "navatala_cfd_predictor_phi_hby_a_fuse",
  "kernel:hip:navatala_cfd_predictor_phi_hby_a_fuse",
  "abi-r1:hip:navatala_cfd_predictor_phi_hby_a_fuse",
  "abi-r1:hip:navatala_cfd_predictor_phi_hby_a_fuse",
  5,
  kAbiArgs_hip_navatala_cfd_predictor_phi_hby_a_fuse
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_axpy_cells[] = {
  { "x", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 4, 4, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_axpy_cells = {
  1,
  "navatala_cfd_axpy_cells",
  "hip",
  "navatala_cfd_axpy_cells",
  "kernel:hip:navatala_cfd_axpy_cells",
  "abi-r1:hip:navatala_cfd_axpy_cells",
  "abi-r1:hip:navatala_cfd_axpy_cells",
  4,
  kAbiArgs_hip_navatala_cfd_axpy_cells
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_ddt_diagonal[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_ddt_diagonal = {
  1,
  "navatala_cfd_ddt_diagonal",
  "hip",
  "navatala_cfd_ddt_diagonal",
  "kernel:hip:navatala_cfd_ddt_diagonal",
  "abi-r1:hip:navatala_cfd_ddt_diagonal",
  "abi-r1:hip:navatala_cfd_ddt_diagonal",
  5,
  kAbiArgs_hip_navatala_cfd_ddt_diagonal
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_ddt_source[] = {
  { "oldField", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "vol0", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 4, 4, 256, nullptr, 0, 0 },
  { "paramsF", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 4, 4, 256, nullptr, 0, 0 },
  { "outSource", 4, KernelArgumentRole::Output, KernelAccessMode::WriteOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_ddt_source = {
  1,
  "navatala_cfd_ddt_source",
  "hip",
  "navatala_cfd_ddt_source",
  "kernel:hip:navatala_cfd_ddt_source",
  "abi-r1:hip:navatala_cfd_ddt_source",
  "abi-r1:hip:navatala_cfd_ddt_source",
  5,
  kAbiArgs_hip_navatala_cfd_ddt_source
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_div_upwind_coeffs[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_div_upwind_coeffs = {
  1,
  "navatala_cfd_div_upwind_coeffs",
  "hip",
  "navatala_cfd_div_upwind_coeffs",
  "kernel:hip:navatala_cfd_div_upwind_coeffs",
  "abi-r1:hip:navatala_cfd_div_upwind_coeffs",
  "abi-r1:hip:navatala_cfd_div_upwind_coeffs",
  5,
  kAbiArgs_hip_navatala_cfd_div_upwind_coeffs
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_fvm_sp[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_fvm_sp = {
  1,
  "navatala_cfd_fvm_sp",
  "hip",
  "navatala_cfd_fvm_sp",
  "kernel:hip:navatala_cfd_fvm_sp",
  "abi-r1:hip:navatala_cfd_fvm_sp",
  "abi-r1:hip:navatala_cfd_fvm_sp",
  4,
  kAbiArgs_hip_navatala_cfd_fvm_sp
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_fvm_su_sp[] = {
  { "sp", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_fvm_su_sp = {
  1,
  "navatala_cfd_fvm_su_sp",
  "hip",
  "navatala_cfd_fvm_su_sp",
  "kernel:hip:navatala_cfd_fvm_su_sp",
  "abi-r1:hip:navatala_cfd_fvm_su_sp",
  "abi-r1:hip:navatala_cfd_fvm_su_sp",
  6,
  kAbiArgs_hip_navatala_cfd_fvm_su_sp
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_laplacian_coeffs[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_laplacian_coeffs = {
  1,
  "navatala_cfd_laplacian_coeffs",
  "hip",
  "navatala_cfd_laplacian_coeffs",
  "kernel:hip:navatala_cfd_laplacian_coeffs",
  "abi-r1:hip:navatala_cfd_laplacian_coeffs",
  "abi-r1:hip:navatala_cfd_laplacian_coeffs",
  9,
  kAbiArgs_hip_navatala_cfd_laplacian_coeffs
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_laplacian_coeffs_accum[] = {
  { "gamma", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_laplacian_coeffs_accum = {
  1,
  "navatala_cfd_laplacian_coeffs_accum",
  "hip",
  "navatala_cfd_laplacian_coeffs_accum",
  "kernel:hip:navatala_cfd_laplacian_coeffs_accum",
  "abi-r1:hip:navatala_cfd_laplacian_coeffs_accum",
  "abi-r1:hip:navatala_cfd_laplacian_coeffs_accum",
  6,
  kAbiArgs_hip_navatala_cfd_laplacian_coeffs_accum
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_linear_upwind_correction_source[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_linear_upwind_correction_source = {
  1,
  "navatala_cfd_linear_upwind_correction_source",
  "hip",
  "navatala_cfd_linear_upwind_correction_source",
  "kernel:hip:navatala_cfd_linear_upwind_correction_source",
  "abi-r1:hip:navatala_cfd_linear_upwind_correction_source",
  "abi-r1:hip:navatala_cfd_linear_upwind_correction_source",
  10,
  kAbiArgs_hip_navatala_cfd_linear_upwind_correction_source
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_matrix_relax[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_matrix_relax = {
  1,
  "navatala_cfd_matrix_relax",
  "hip",
  "navatala_cfd_matrix_relax",
  "kernel:hip:navatala_cfd_matrix_relax",
  "abi-r1:hip:navatala_cfd_matrix_relax",
  "abi-r1:hip:navatala_cfd_matrix_relax",
  7,
  kAbiArgs_hip_navatala_cfd_matrix_relax
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_neg_sum_diag[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_neg_sum_diag = {
  1,
  "navatala_cfd_neg_sum_diag",
  "hip",
  "navatala_cfd_neg_sum_diag",
  "kernel:hip:navatala_cfd_neg_sum_diag",
  "abi-r1:hip:navatala_cfd_neg_sum_diag",
  "abi-r1:hip:navatala_cfd_neg_sum_diag",
  7,
  kAbiArgs_hip_navatala_cfd_neg_sum_diag
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_pack_boundary_adjacent_cells[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_pack_boundary_adjacent_cells = {
  1,
  "navatala_cfd_pack_boundary_adjacent_cells",
  "hip",
  "navatala_cfd_pack_boundary_adjacent_cells",
  "kernel:hip:navatala_cfd_pack_boundary_adjacent_cells",
  "abi-r1:hip:navatala_cfd_pack_boundary_adjacent_cells",
  "abi-r1:hip:navatala_cfd_pack_boundary_adjacent_cells",
  4,
  kAbiArgs_hip_navatala_cfd_pack_boundary_adjacent_cells
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_scalar_jacobi_clamp_min[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_scalar_jacobi_clamp_min = {
  1,
  "navatala_cfd_scalar_jacobi_clamp_min",
  "hip",
  "navatala_cfd_scalar_jacobi_clamp_min",
  "kernel:hip:navatala_cfd_scalar_jacobi_clamp_min",
  "abi-r1:hip:navatala_cfd_scalar_jacobi_clamp_min",
  "abi-r1:hip:navatala_cfd_scalar_jacobi_clamp_min",
  3,
  kAbiArgs_hip_navatala_cfd_scalar_jacobi_clamp_min
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_scalar_jacobi_update[] = {
  { "ax", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_scalar_jacobi_update = {
  1,
  "navatala_cfd_scalar_jacobi_update",
  "hip",
  "navatala_cfd_scalar_jacobi_update",
  "kernel:hip:navatala_cfd_scalar_jacobi_update",
  "abi-r1:hip:navatala_cfd_scalar_jacobi_update",
  "abi-r1:hip:navatala_cfd_scalar_jacobi_update",
  6,
  kAbiArgs_hip_navatala_cfd_scalar_jacobi_update
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_scalar_ldu_coupled_interface_add[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_scalar_ldu_coupled_interface_add = {
  1,
  "navatala_cfd_scalar_ldu_coupled_interface_add",
  "hip",
  "navatala_cfd_scalar_ldu_coupled_interface_add",
  "kernel:hip:navatala_cfd_scalar_ldu_coupled_interface_add",
  "abi-r1:hip:navatala_cfd_scalar_ldu_coupled_interface_add",
  "abi-r1:hip:navatala_cfd_scalar_ldu_coupled_interface_add",
  5,
  kAbiArgs_hip_navatala_cfd_scalar_ldu_coupled_interface_add
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_scalar_ldu_mat_vec[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_scalar_ldu_mat_vec = {
  1,
  "navatala_cfd_scalar_ldu_mat_vec",
  "hip",
  "navatala_cfd_scalar_ldu_mat_vec",
  "kernel:hip:navatala_cfd_scalar_ldu_mat_vec",
  "abi-r1:hip:navatala_cfd_scalar_ldu_mat_vec",
  "abi-r1:hip:navatala_cfd_scalar_ldu_mat_vec",
  11,
  kAbiArgs_hip_navatala_cfd_scalar_ldu_mat_vec
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_scalar_ldu_multi_color_gs_sweep[] = {
  { "diag", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "upper", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "lower", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_scalar_ldu_multi_color_gs_sweep = {
  1,
  "navatala_cfd_scalar_ldu_multi_color_gs_sweep",
  "hip",
  "navatala_cfd_scalar_ldu_multi_color_gs_sweep",
  "kernel:hip:navatala_cfd_scalar_ldu_multi_color_gs_sweep",
  "abi-r1:hip:navatala_cfd_scalar_ldu_multi_color_gs_sweep",
  "abi-r1:hip:navatala_cfd_scalar_ldu_multi_color_gs_sweep",
  14,
  kAbiArgs_hip_navatala_cfd_scalar_ldu_multi_color_gs_sweep
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_sum_mag_off_diag[] = {
  { "upper", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "lower", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "offsets", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "faceIdx", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_sum_mag_off_diag = {
  1,
  "navatala_cfd_sum_mag_off_diag",
  "hip",
  "navatala_cfd_sum_mag_off_diag",
  "kernel:hip:navatala_cfd_sum_mag_off_diag",
  "abi-r1:hip:navatala_cfd_sum_mag_off_diag",
  "abi-r1:hip:navatala_cfd_sum_mag_off_diag",
  7,
  kAbiArgs_hip_navatala_cfd_sum_mag_off_diag
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_upwind_weights[] = {
  ...
  { "outWeights", 2, KernelArgumentRole::Output, KernelAccessMode::WriteOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_upwind_weights = {
  1,
  "navatala_cfd_upwind_weights",
  "hip",
  "navatala_cfd_upwind_weights",
  "kernel:hip:navatala_cfd_upwind_weights",
  "abi-r1:hip:navatala_cfd_upwind_weights",
  "abi-r1:hip:navatala_cfd_upwind_weights",
  3,
  kAbiArgs_hip_navatala_cfd_upwind_weights
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_gamg_axpy_in_place[] = {
  { "x", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_gamg_axpy_in_place = {
  1,
  "navatala_cfd_gamg_axpy_in_place",
  "hip",
  "navatala_cfd_gamg_axpy_in_place",
  "kernel:hip:navatala_cfd_gamg_axpy_in_place",
  "abi-r1:hip:navatala_cfd_gamg_axpy_in_place",
  "abi-r1:hip:navatala_cfd_gamg_axpy_in_place",
  4,
  kAbiArgs_hip_navatala_cfd_gamg_axpy_in_place
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_gamg_compute_scale_factor[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_gamg_compute_scale_factor = {
  1,
  "navatala_cfd_gamg_compute_scale_factor",
  "hip",
  "navatala_cfd_gamg_compute_scale_factor",
  "kernel:hip:navatala_cfd_gamg_compute_scale_factor",
  "abi-r1:hip:navatala_cfd_gamg_compute_scale_factor",
  "abi-r1:hip:navatala_cfd_gamg_compute_scale_factor",
  3,
  kAbiArgs_hip_navatala_cfd_gamg_compute_scale_factor
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_gamg_dot_finalize[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_gamg_dot_finalize = {
  1,
  "navatala_cfd_gamg_dot_finalize",
  "hip",
  "navatala_cfd_gamg_dot_finalize",
  "kernel:hip:navatala_cfd_gamg_dot_finalize",
  "abi-r1:hip:navatala_cfd_gamg_dot_finalize",
  "abi-r1:hip:navatala_cfd_gamg_dot_finalize",
  3,
  kAbiArgs_hip_navatala_cfd_gamg_dot_finalize
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_gamg_negate_scalar[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_gamg_negate_scalar = {
  1,
  "navatala_cfd_gamg_negate_scalar",
  "hip",
  "navatala_cfd_gamg_negate_scalar",
  "kernel:hip:navatala_cfd_gamg_negate_scalar",
  "abi-r1:hip:navatala_cfd_gamg_negate_scalar",
  "abi-r1:hip:navatala_cfd_gamg_negate_scalar",
  2,
  kAbiArgs_hip_navatala_cfd_gamg_negate_scalar
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_gamg_scale_correction[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_gamg_scale_correction = {
  1,
  "navatala_cfd_gamg_scale_correction",
  "hip",
  "navatala_cfd_gamg_scale_correction",
  "kernel:hip:navatala_cfd_gamg_scale_correction",
  "abi-r1:hip:navatala_cfd_gamg_scale_correction",
  "abi-r1:hip:navatala_cfd_gamg_scale_correction",
  7,
  kAbiArgs_hip_navatala_cfd_gamg_scale_correction
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mg_dic_build[] = {
  { "diag", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mg_dic_build = {
  1,
  "navatala_cfd_mg_dic_build",
  "hip",
  "navatala_cfd_mg_dic_build",
  "kernel:hip:navatala_cfd_mg_dic_build",
  "abi-r1:hip:navatala_cfd_mg_dic_build",
  "abi-r1:hip:navatala_cfd_mg_dic_build",
  6,
  kAbiArgs_hip_navatala_cfd_mg_dic_build
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mg_dic_smooth[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mg_dic_smooth = {
  1,
  "navatala_cfd_mg_dic_smooth",
  "hip",
  "navatala_cfd_mg_dic_smooth",
  "kernel:hip:navatala_cfd_mg_dic_smooth",
  "abi-r1:hip:navatala_cfd_mg_dic_smooth",
  "abi-r1:hip:navatala_cfd_mg_dic_smooth",
  9,
  kAbiArgs_hip_navatala_cfd_mg_dic_smooth
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mg_coarse_jacobi[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mg_coarse_jacobi = {
  1,
  "navatala_cfd_mg_coarse_jacobi",
  "hip",
  "navatala_cfd_mg_coarse_jacobi",
  "kernel:hip:navatala_cfd_mg_coarse_jacobi",
  "abi-r1:hip:navatala_cfd_mg_coarse_jacobi",
  "abi-r1:hip:navatala_cfd_mg_coarse_jacobi",
  4,
  kAbiArgs_hip_navatala_cfd_mg_coarse_jacobi
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mg_coarse_matvec[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mg_coarse_matvec = {
  1,
  "navatala_cfd_mg_coarse_matvec",
  "hip",
  "navatala_cfd_mg_coarse_matvec",
  "kernel:hip:navatala_cfd_mg_coarse_matvec",
  "abi-r1:hip:navatala_cfd_mg_coarse_matvec",
  "abi-r1:hip:navatala_cfd_mg_coarse_matvec",
  7,
  kAbiArgs_hip_navatala_cfd_mg_coarse_matvec
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mg_coarse_rhs[] = {
  { "b", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "ax", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "r", 2, KernelArgumentRole::InputOutput, KernelAccessMode::ReadWrite, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "mgCounts", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mg_coarse_rhs = {
  1,
  "navatala_cfd_mg_coarse_rhs",
  "hip",
  "navatala_cfd_mg_coarse_rhs",
  "kernel:hip:navatala_cfd_mg_coarse_rhs",
  "abi-r1:hip:navatala_cfd_mg_coarse_rhs",
  "abi-r1:hip:navatala_cfd_mg_coarse_rhs",
  4,
  kAbiArgs_hip_navatala_cfd_mg_coarse_rhs
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mg_find_best_match[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mg_find_best_match = {
  1,
  "navatala_cfd_mg_find_best_match",
  "hip",
  "navatala_cfd_mg_find_best_match",
  "kernel:hip:navatala_cfd_mg_find_best_match",
  "abi-r1:hip:navatala_cfd_mg_find_best_match",
  "abi-r1:hip:navatala_cfd_mg_find_best_match",
  8,
  kAbiArgs_hip_navatala_cfd_mg_find_best_match
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mg_pair_match[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mg_pair_match = {
  1,
  "navatala_cfd_mg_pair_match",
  "hip",
  "navatala_cfd_mg_pair_match",
  "kernel:hip:navatala_cfd_mg_pair_match",
  "abi-r1:hip:navatala_cfd_mg_pair_match",
  "abi-r1:hip:navatala_cfd_mg_pair_match",
  3,
  kAbiArgs_hip_navatala_cfd_mg_pair_match
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mg_prolong_add[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mg_prolong_add = {
  1,
  "navatala_cfd_mg_prolong_add",
  "hip",
  "navatala_cfd_mg_prolong_add",
  "kernel:hip:navatala_cfd_mg_prolong_add",
  "abi-r1:hip:navatala_cfd_mg_prolong_add",
  "abi-r1:hip:navatala_cfd_mg_prolong_add",
  6,
  kAbiArgs_hip_navatala_cfd_mg_prolong_add
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mg_renumber_aggregates[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mg_renumber_aggregates = {
  1,
  "navatala_cfd_mg_renumber_aggregates",
  "hip",
  "navatala_cfd_mg_renumber_aggregates",
  "kernel:hip:navatala_cfd_mg_renumber_aggregates",
  "abi-r1:hip:navatala_cfd_mg_renumber_aggregates",
  "abi-r1:hip:navatala_cfd_mg_renumber_aggregates",
  3,
  kAbiArgs_hip_navatala_cfd_mg_renumber_aggregates
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mg_renumber_propagate[] = {
  { "aggMap", 0, KernelArgumentRole::InputOutput, KernelAccessMode::ReadWrite, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mg_renumber_propagate = {
  1,
  "navatala_cfd_mg_renumber_propagate",
  "hip",
  "navatala_cfd_mg_renumber_propagate",
  "kernel:hip:navatala_cfd_mg_renumber_propagate",
  "abi-r1:hip:navatala_cfd_mg_renumber_propagate",
  "abi-r1:hip:navatala_cfd_mg_renumber_propagate",
  2,
  kAbiArgs_hip_navatala_cfd_mg_renumber_propagate
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mg_resolve_chains[] = {
  { "aggMap", 0, KernelArgumentRole::InputOutput, KernelAccessMode::ReadWrite, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mg_resolve_chains = {
  1,
  "navatala_cfd_mg_resolve_chains",
  "hip",
  "navatala_cfd_mg_resolve_chains",
  "kernel:hip:navatala_cfd_mg_resolve_chains",
  "abi-r1:hip:navatala_cfd_mg_resolve_chains",
  "abi-r1:hip:navatala_cfd_mg_resolve_chains",
  2,
  kAbiArgs_hip_navatala_cfd_mg_resolve_chains
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mg_restrict_sum[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mg_restrict_sum = {
  1,
  "navatala_cfd_mg_restrict_sum",
  "hip",
  "navatala_cfd_mg_restrict_sum",
  "kernel:hip:navatala_cfd_mg_restrict_sum",
  "abi-r1:hip:navatala_cfd_mg_restrict_sum",
  "abi-r1:hip:navatala_cfd_mg_restrict_sum",
  6,
  kAbiArgs_hip_navatala_cfd_mg_restrict_sum
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mg_strength_of_connection[] = {
  { "diag", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "upper", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "owner", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mg_strength_of_connection = {
  1,
  "navatala_cfd_mg_strength_of_connection",
  "hip",
  "navatala_cfd_mg_strength_of_connection",
  "kernel:hip:navatala_cfd_mg_strength_of_connection",
  "abi-r1:hip:navatala_cfd_mg_strength_of_connection",
  "abi-r1:hip:navatala_cfd_mg_strength_of_connection",
  6,
  kAbiArgs_hip_navatala_cfd_mg_strength_of_connection
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mg_zero_coarse[] = {
  { "x", 0, KernelArgumentRole::InputOutput, KernelAccessMode::ReadWrite, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "mgCounts", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mg_zero_coarse = {
  1,
  "navatala_cfd_mg_zero_coarse",
  "hip",
  "navatala_cfd_mg_zero_coarse",
  "kernel:hip:navatala_cfd_mg_zero_coarse",
  "abi-r1:hip:navatala_cfd_mg_zero_coarse",
  "abi-r1:hip:navatala_cfd_mg_zero_coarse",
  2,
  kAbiArgs_hip_navatala_cfd_mg_zero_coarse
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_phi_correct[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_phi_correct = {
  1,
  "navatala_cfd_phi_correct",
  "hip",
  "navatala_cfd_phi_correct",
  "kernel:hip:navatala_cfd_phi_correct",
  "abi-r1:hip:navatala_cfd_phi_correct",
  "abi-r1:hip:navatala_cfd_phi_correct",
  15,
  kAbiArgs_hip_navatala_cfd_phi_correct
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_phi_correct_boundary[] = {
  { "phiIn", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "pCell", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "owner", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_phi_correct_boundary = {
  1,
  "navatala_cfd_phi_correct_boundary",
  "hip",
  "navatala_cfd_phi_correct_boundary",
  "kernel:hip:navatala_cfd_phi_correct_boundary",
  "abi-r1:hip:navatala_cfd_phi_correct_boundary",
  "abi-r1:hip:navatala_cfd_phi_correct_boundary",
  14,
  kAbiArgs_hip_navatala_cfd_phi_correct_boundary
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_phi_correct_internal[] = {
  { "phiIn", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "pCell", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "owner", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "neighbour", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_phi_correct_internal = {
  1,
  "navatala_cfd_phi_correct_internal",
  "hip",
  "navatala_cfd_phi_correct_internal",
  "kernel:hip:navatala_cfd_phi_correct_internal",
  "abi-r1:hip:navatala_cfd_phi_correct_internal",
  "abi-r1:hip:navatala_cfd_phi_correct_internal",
  11,
  kAbiArgs_hip_navatala_cfd_phi_correct_internal
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_axpy_in_place[] = {
  { "x", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_axpy_in_place = {
  1,
  "navatala_cfd_axpy_in_place",
  "hip",
  "navatala_cfd_axpy_in_place",
  "kernel:hip:navatala_cfd_axpy_in_place",
  "abi-r1:hip:navatala_cfd_axpy_in_place",
  "abi-r1:hip:navatala_cfd_axpy_in_place",
  4,
  kAbiArgs_hip_navatala_cfd_axpy_in_place
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_bc_dirichlet_face_flux[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_bc_dirichlet_face_flux = {
  1,
  "navatala_cfd_bc_dirichlet_face_flux",
  "hip",
  "navatala_cfd_bc_dirichlet_face_flux",
  "kernel:hip:navatala_cfd_bc_dirichlet_face_flux",
  "abi-r1:hip:navatala_cfd_bc_dirichlet_face_flux",
  "abi-r1:hip:navatala_cfd_bc_dirichlet_face_flux",
  5,
  kAbiArgs_hip_navatala_cfd_bc_dirichlet_face_flux
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_bc_sn_grad_face_flux[] = {
  { "cf", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_bc_sn_grad_face_flux = {
  1,
  "navatala_cfd_bc_sn_grad_face_flux",
  "hip",
  "navatala_cfd_bc_sn_grad_face_flux",
  "kernel:hip:navatala_cfd_bc_sn_grad_face_flux",
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_coeff_to_cf_in_place = {
  1,
  "navatala_cfd_coeff_to_cf_in_place",
  "hip",
  "navatala_cfd_coeff_to_cf_in_place",
  "kernel:hip:navatala_cfd_coeff_to_cf_in_place",
  "abi-r1:hip:navatala_cfd_coeff_to_cf_in_place",
  "abi-r1:hip:navatala_cfd_coeff_to_cf_in_place",
  4,
  kAbiArgs_hip_navatala_cfd_coeff_to_cf_in_place
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_diag_from_cf[] = {
  { "cf", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_diag_from_cf = {
  1,
  "navatala_cfd_diag_from_cf",
  "hip",
  "navatala_cfd_diag_from_cf",
  "kernel:hip:navatala_cfd_diag_from_cf",
  "abi-r1:hip:navatala_cfd_diag_from_cf",
  "abi-r1:hip:navatala_cfd_diag_from_cf",
  6,
  kAbiArgs_hip_navatala_cfd_diag_from_cf
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_dic_apply[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_dic_apply = {
  1,
  "navatala_cfd_dic_apply",
  "hip",
  "navatala_cfd_dic_apply",
  "kernel:hip:navatala_cfd_dic_apply",
  "abi-r1:hip:navatala_cfd_dic_apply",
  "abi-r1:hip:navatala_cfd_dic_apply",
  7,
  kAbiArgs_hip_navatala_cfd_dic_apply
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_dic_build_reciprocal_d[] = {
  { "diag", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "upper", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "owner", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "neighbour", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_dic_build_reciprocal_d = {
  1,
  "navatala_cfd_dic_build_reciprocal_d",
  "hip",
  "navatala_cfd_dic_build_reciprocal_d",
  "kernel:hip:navatala_cfd_dic_build_reciprocal_d",
  "abi-r1:hip:navatala_cfd_dic_build_reciprocal_d",
  "abi-r1:hip:navatala_cfd_dic_build_reciprocal_d",
  6,
  kAbiArgs_hip_navatala_cfd_dic_build_reciprocal_d
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_dot_partials[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_dot_partials = {
  1,
  "navatala_cfd_dot_partials",
  "hip",
  "navatala_cfd_dot_partials",
  "kernel:hip:navatala_cfd_dot_partials",
  "abi-r1:hip:navatala_cfd_dot_partials",
  "abi-r1:hip:navatala_cfd_dot_partials",
  4,
  kAbiArgs_hip_navatala_cfd_dot_partials
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_face_flux[] = {
  { "x", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "owner", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "neighbour", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_face_flux = {
  1,
  "navatala_cfd_face_flux",
  "hip",
  "navatala_cfd_face_flux",
  "kernel:hip:navatala_cfd_face_flux",
  "abi-r1:hip:navatala_cfd_face_flux",
  "abi-r1:hip:navatala_cfd_face_flux",
  8,
  kAbiArgs_hip_navatala_cfd_face_flux
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_inv_diag[] = {
  { "r", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "diag", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 12, 12, 256, nullptr, 0, 0 },
  { "z", 3, KernelArgumentRole::Output, KernelAccessMode::WriteOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_inv_diag = {
  1,
  "navatala_cfd_inv_diag",
  "hip",
  "navatala_cfd_inv_diag",
  "kernel:hip:navatala_cfd_inv_diag",
  "abi-r1:hip:navatala_cfd_inv_diag",
  "abi-r1:hip:navatala_cfd_inv_diag",
  4,
  kAbiArgs_hip_navatala_cfd_inv_diag
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_laplacian_from_face_flux[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_laplacian_from_face_flux = {
  1,
  "navatala_cfd_laplacian_from_face_flux",
  "hip",
  "navatala_cfd_laplacian_from_face_flux",
  "kernel:hip:navatala_cfd_laplacian_from_face_flux",
  "abi-r1:hip:navatala_cfd_laplacian_from_face_flux",
  "abi-r1:hip:navatala_cfd_laplacian_from_face_flux",
  7,
  kAbiArgs_hip_navatala_cfd_laplacian_from_face_flux
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mul_by_vol_in_place[] = {
  { "b", 0, KernelArgumentRole::InputOutput, KernelAccessMode::ReadWrite, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "vol", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 12, 12, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mul_by_vol_in_place = {
  1,
  "navatala_cfd_mul_by_vol_in_place",
  "hip",
  "navatala_cfd_mul_by_vol_in_place",
  "kernel:hip:navatala_cfd_mul_by_vol_in_place",
  "abi-r1:hip:navatala_cfd_mul_by_vol_in_place",
  "abi-r1:hip:navatala_cfd_mul_by_vol_in_place",
  3,
  kAbiArgs_hip_navatala_cfd_mul_by_vol_in_place
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_ref_add_ax[] = {
  { "diag", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_ref_add_ax = {
  1,
  "navatala_cfd_ref_add_ax",
  "hip",
  "navatala_cfd_ref_add_ax",
  "kernel:hip:navatala_cfd_ref_add_ax",
  "abi-r1:hip:navatala_cfd_ref_add_ax",
  "abi-r1:hip:navatala_cfd_ref_add_ax",
  4,
  kAbiArgs_hip_navatala_cfd_ref_add_ax
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_ref_add_ax_native_f64[] = {
  { "diag", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "x", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "outAx", 2, KernelArgumentRole::InputOutput, KernelAccessMode::ReadWrite, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "refCell", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_ref_add_ax_native_f64 = {
  1,
  "navatala_cfd_ref_add_ax_native_f64",
  "hip",
  "navatala_cfd_ref_add_ax_native_f64",
  "kernel:hip:navatala_cfd_ref_add_ax_native_f64",
  "abi-r1:hip:navatala_cfd_ref_add_ax_native_f64",
  "abi-r1:hip:navatala_cfd_ref_add_ax_native_f64",
  4,
  kAbiArgs_hip_navatala_cfd_ref_add_ax_native_f64
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_ref_add_b[] = {
  { "diag", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_ref_add_b = {
  1,
  "navatala_cfd_ref_add_b",
  "hip",
  "navatala_cfd_ref_add_b",
  "kernel:hip:navatala_cfd_ref_add_b",
  "abi-r1:hip:navatala_cfd_ref_add_b",
  "abi-r1:hip:navatala_cfd_ref_add_b",
  4,
  kAbiArgs_hip_navatala_cfd_ref_add_b
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_ref_add_b_native_f64[] = {
  { "diag", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "b", 1, KernelArgumentRole::InputOutput, KernelAccessMode::ReadWrite, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "refCell", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "refVal", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 8, 8, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_ref_add_b_native_f64 = {
  1,
  "navatala_cfd_ref_add_b_native_f64",
  "hip",
  "navatala_cfd_ref_add_b_native_f64",
  "kernel:hip:navatala_cfd_ref_add_b_native_f64",
  "abi-r1:hip:navatala_cfd_ref_add_b_native_f64",
  "abi-r1:hip:navatala_cfd_ref_add_b_native_f64",
  4,
  kAbiArgs_hip_navatala_cfd_ref_add_b_native_f64
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_rhs_sub[] = {
  { "b", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "ax", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 12, 12, 256, nullptr, 0, 0 },
  { "r", 3, KernelArgumentRole::Output, KernelAccessMode::WriteOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_rhs_sub = {
  1,
  "navatala_cfd_rhs_sub",
  "hip",
  "navatala_cfd_rhs_sub",
  "kernel:hip:navatala_cfd_rhs_sub",
  "abi-r1:hip:navatala_cfd_rhs_sub",
  "abi-r1:hip:navatala_cfd_rhs_sub",
  4,
  kAbiArgs_hip_navatala_cfd_rhs_sub
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_shift_in_place[] = {
  { "x", 0, KernelArgumentRole::InputOutput, KernelAccessMode::ReadWrite, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 12, 12, 256, nullptr, 0, 0 },
  { "paramsF", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 4, 4, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_shift_in_place = {
  1,
  "navatala_cfd_shift_in_place",
  "hip",
  "navatala_cfd_shift_in_place",
  "kernel:hip:navatala_cfd_shift_in_place",
  "abi-r1:hip:navatala_cfd_shift_in_place",
  "abi-r1:hip:navatala_cfd_shift_in_place",
  3,
  kAbiArgs_hip_navatala_cfd_shift_in_place
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_sum_abs_partials[] = {
  { "a", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 12, 12, 256, nullptr, 0, 0 },
  { "outPartials", 2, KernelArgumentRole::Output, KernelAccessMode::WriteOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_sum_abs_partials = {
  1,
  "navatala_cfd_sum_abs_partials",
  "hip",
  "navatala_cfd_sum_abs_partials",
  "kernel:hip:navatala_cfd_sum_abs_partials",
  "abi-r1:hip:navatala_cfd_sum_abs_partials",
  "abi-r1:hip:navatala_cfd_sum_abs_partials",
  3,
  kAbiArgs_hip_navatala_cfd_sum_abs_partials
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_upper_from_cf[] = {
  { "cf", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "counts", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 12, 12, 256, nullptr, 0, 0 },
  { "upper", 2, KernelArgumentRole::Output, KernelAccessMode::WriteOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_upper_from_cf = {
  1,
  "navatala_cfd_upper_from_cf",
  "hip",
  "navatala_cfd_upper_from_cf",
  "kernel:hip:navatala_cfd_upper_from_cf",
  "abi-r1:hip:navatala_cfd_upper_from_cf",
  "abi-r1:hip:navatala_cfd_upper_from_cf",
  3,
  kAbiArgs_hip_navatala_cfd_upper_from_cf
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_xpay_in_place[] = {
  { "z", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_xpay_in_place = {
  1,
  "navatala_cfd_xpay_in_place",
  "hip",
  "navatala_cfd_xpay_in_place",
  "kernel:hip:navatala_cfd_xpay_in_place",
  "abi-r1:hip:navatala_cfd_xpay_in_place",
  "abi-r1:hip:navatala_cfd_xpay_in_place",
  5,
  kAbiArgs_hip_navatala_cfd_xpay_in_place
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_dot_finalize_partials[] = {
  { "partials", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "count", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 4, 4, 256, nullptr, 0, 0 },
  { "result", 2, KernelArgumentRole::Output, KernelAccessMode::WriteOnly, GpuRuntime::MemoryKind::Device, true, 8, 8, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_dot_finalize_partials = {
  1,
  "navatala_cfd_dot_finalize_partials",
  "hip",
  "navatala_cfd_dot_finalize_partials",
  "kernel:hip:navatala_cfd_dot_finalize_partials",
  "abi-r1:hip:navatala_cfd_dot_finalize_partials",
  "abi-r1:hip:navatala_cfd_dot_finalize_partials",
  3,
  kAbiArgs_hip_navatala_cfd_dot_finalize_partials
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_negate_scalar[] = {
  { "input", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 4, 4, 256, nullptr, 0, 0 },
  { "output", 1, KernelArgumentRole::Output, KernelAccessMode::WriteOnly, GpuRuntime::MemoryKind::Device, true, 4, 4, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_negate_scalar = {
  1,
  "navatala_cfd_negate_scalar",
  "hip",
  "navatala_cfd_negate_scalar",
  "kernel:hip:navatala_cfd_negate_scalar",
  "abi-r1:hip:navatala_cfd_negate_scalar",
  "abi-r1:hip:navatala_cfd_negate_scalar",
  2,
  kAbiArgs_hip_navatala_cfd_negate_scalar
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_pcg_compute_alpha[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_pcg_compute_alpha = {
  1,
  "navatala_cfd_pcg_compute_alpha",
  "hip",
  "navatala_cfd_pcg_compute_alpha",
  "kernel:hip:navatala_cfd_pcg_compute_alpha",
  "abi-r1:hip:navatala_cfd_pcg_compute_alpha",
  "abi-r1:hip:navatala_cfd_pcg_compute_alpha",
  3,
  kAbiArgs_hip_navatala_cfd_pcg_compute_alpha
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_pcg_compute_beta[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_pcg_compute_beta = {
  1,
  "navatala_cfd_pcg_compute_beta",
  "hip",
  "navatala_cfd_pcg_compute_beta",
  "kernel:hip:navatala_cfd_pcg_compute_beta",
  "abi-r1:hip:navatala_cfd_pcg_compute_beta",
  "abi-r1:hip:navatala_cfd_pcg_compute_beta",
  3,
  kAbiArgs_hip_navatala_cfd_pcg_compute_beta
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_exact_mat_vec[] = {
  { "x", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "owner", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "neighbour", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_exact_mat_vec = {
  1,
  "navatala_cfd_exact_mat_vec",
  "hip",
  "navatala_cfd_exact_mat_vec",
  "kernel:hip:navatala_cfd_exact_mat_vec",
  "abi-r1:hip:navatala_cfd_exact_mat_vec",
  "abi-r1:hip:navatala_cfd_exact_mat_vec",
  11,
  kAbiArgs_hip_navatala_cfd_exact_mat_vec
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_exact_mat_vec_f64[] = {
  { "x", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "owner", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "neighbour", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "offsets", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_exact_mat_vec_f64 = {
  1,
  "navatala_cfd_exact_mat_vec_f64",
  "hip",
  "navatala_cfd_exact_mat_vec_f64",
  "kernel:hip:navatala_cfd_exact_mat_vec_f64",
  "abi-r1:hip:navatala_cfd_exact_mat_vec_f64",
  "abi-r1:hip:navatala_cfd_exact_mat_vec_f64",
  11,
  kAbiArgs_hip_navatala_cfd_exact_mat_vec_f64
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_exact_mat_vec_native_f64[] = {
  { "x", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "owner", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "neighbour", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "offsets", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_exact_mat_vec_native_f64 = {
  1,
  "navatala_cfd_exact_mat_vec_native_f64",
  "hip",
  "navatala_cfd_exact_mat_vec_native_f64",
  "kernel:hip:navatala_cfd_exact_mat_vec_native_f64",
  "abi-r1:hip:navatala_cfd_exact_mat_vec_native_f64",
  "abi-r1:hip:navatala_cfd_exact_mat_vec_native_f64",
  11,
  kAbiArgs_hip_navatala_cfd_exact_mat_vec_native_f64
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_face_flux_boundary[] = {
  { "x", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "owner", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_face_flux_boundary = {
  1,
  "navatala_cfd_face_flux_boundary",
  "hip",
  "navatala_cfd_face_flux_boundary",
  "kernel:hip:navatala_cfd_face_flux_boundary",
  "abi-r1:hip:navatala_cfd_face_flux_boundary",
  "abi-r1:hip:navatala_cfd_face_flux_boundary",
  7,
  kAbiArgs_hip_navatala_cfd_face_flux_boundary
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_face_flux_internal[] = {
  { "x", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "owner", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "neighbour", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "cf", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_face_flux_internal = {
  1,
  "navatala_cfd_face_flux_internal",
  "hip",
  "navatala_cfd_face_flux_internal",
  "kernel:hip:navatala_cfd_face_flux_internal",
  "abi-r1:hip:navatala_cfd_face_flux_internal",
  "abi-r1:hip:navatala_cfd_face_flux_internal",
  6,
  kAbiArgs_hip_navatala_cfd_face_flux_internal
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_pressure_face_flux_correction_corrected[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_pressure_face_flux_correction_corrected = {
  1,
  "navatala_cfd_pressure_face_flux_correction_corrected",
  "hip",
  "navatala_cfd_pressure_face_flux_correction_corrected",
  "kernel:hip:navatala_cfd_pressure_face_flux_correction_corrected",
  "abi-r1:hip:navatala_cfd_pressure_face_flux_correction_corrected",
  "abi-r1:hip:navatala_cfd_pressure_face_flux_correction_corrected",
  13,
  kAbiArgs_hip_navatala_cfd_pressure_face_flux_correction_corrected
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_pressure_sn_grad[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_pressure_sn_grad = {
  1,
  "navatala_cfd_pressure_sn_grad",
  "hip",
  "navatala_cfd_pressure_sn_grad",
  "kernel:hip:navatala_cfd_pressure_sn_grad",
  "abi-r1:hip:navatala_cfd_pressure_sn_grad",
  "abi-r1:hip:navatala_cfd_pressure_sn_grad",
  17,
  kAbiArgs_hip_navatala_cfd_pressure_sn_grad
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_u_correct[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_u_correct = {
  1,
  "navatala_cfd_u_correct",
  "hip",
  "navatala_cfd_u_correct",
  "kernel:hip:navatala_cfd_u_correct",
  "abi-r1:hip:navatala_cfd_u_correct",
  "abi-r1:hip:navatala_cfd_u_correct",
  21,
  kAbiArgs_hip_navatala_cfd_u_correct
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_u_correct_reconstruct[] = {
  { "pCell", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_u_correct_reconstruct = {
  1,
  "navatala_cfd_u_correct_reconstruct",
  "hip",
  "navatala_cfd_u_correct_reconstruct",
  "kernel:hip:navatala_cfd_u_correct_reconstruct",
  "abi-r1:hip:navatala_cfd_u_correct_reconstruct",
  "abi-r1:hip:navatala_cfd_u_correct_reconstruct",
  28,
  kAbiArgs_hip_navatala_cfd_u_correct_reconstruct
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_grad_alpha_cell[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_grad_alpha_cell = {
  1,
  "navatala_cfd_grad_alpha_cell",
  "hip",
  "navatala_cfd_grad_alpha_cell",
  "kernel:hip:navatala_cfd_grad_alpha_cell",
  "abi-r1:hip:navatala_cfd_grad_alpha_cell",
  "abi-r1:hip:navatala_cfd_grad_alpha_cell",
  12,
  kAbiArgs_hip_navatala_cfd_grad_alpha_cell
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_grad_alpha_cell_f64[] = {
  { "alphaF", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "sfX", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "sfY", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "sfZ", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_grad_alpha_cell_f64 = {
  1,
  "navatala_cfd_grad_alpha_cell_f64",
  "hip",
  "navatala_cfd_grad_alpha_cell_f64",
  "kernel:hip:navatala_cfd_grad_alpha_cell_f64",
  "abi-r1:hip:navatala_cfd_grad_alpha_cell_f64",
  "abi-r1:hip:navatala_cfd_grad_alpha_cell_f64",
  12,
  kAbiArgs_hip_navatala_cfd_grad_alpha_cell_f64
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_n_hatf_all[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_n_hatf_all = {
  1,
  "navatala_cfd_n_hatf_all",
  "hip",
  "navatala_cfd_n_hatf_all",
  "kernel:hip:navatala_cfd_n_hatf_all",
  "abi-r1:hip:navatala_cfd_n_hatf_all",
  "abi-r1:hip:navatala_cfd_n_hatf_all",
  15,
  kAbiArgs_hip_navatala_cfd_n_hatf_all
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_n_hatf_all_f64[] = {
  { "gx", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "gy", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "gz", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "sfX", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_n_hatf_all_f64 = {
  1,
  "navatala_cfd_n_hatf_all_f64",
  "hip",
  "navatala_cfd_n_hatf_all_f64",
  "kernel:hip:navatala_cfd_n_hatf_all_f64",
  "abi-r1:hip:navatala_cfd_n_hatf_all_f64",
  "abi-r1:hip:navatala_cfd_n_hatf_all_f64",
  15,
  kAbiArgs_hip_navatala_cfd_n_hatf_all_f64
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_stf_all_faces[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_stf_all_faces = {
  1,
  "navatala_cfd_stf_all_faces",
  "hip",
  "navatala_cfd_stf_all_faces",
  "kernel:hip:navatala_cfd_stf_all_faces",
  "abi-r1:hip:navatala_cfd_stf_all_faces",
  "abi-r1:hip:navatala_cfd_stf_all_faces",
  12,
  kAbiArgs_hip_navatala_cfd_stf_all_faces
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_stf_all_faces_f64[] = {
  { "divN", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "alpha", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "alphaF", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "delta", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_stf_all_faces_f64 = {
  1,
  "navatala_cfd_stf_all_faces_f64",
  "hip",
  "navatala_cfd_stf_all_faces_f64",
  "kernel:hip:navatala_cfd_stf_all_faces_f64",
  "abi-r1:hip:navatala_cfd_stf_all_faces_f64",
  "abi-r1:hip:navatala_cfd_stf_all_faces_f64",
  12,
  kAbiArgs_hip_navatala_cfd_stf_all_faces_f64
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_k_omega_s_s_t_blending[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_k_omega_s_s_t_blending = {
  1,
  "navatala_cfd_k_omega_s_s_t_blending",
  "hip",
  "navatala_cfd_k_omega_s_s_t_blending",
  "kernel:hip:navatala_cfd_k_omega_s_s_t_blending",
  "abi-r1:hip:navatala_cfd_k_omega_s_s_t_blending",
  "abi-r1:hip:navatala_cfd_k_omega_s_s_t_blending",
  15,
  kAbiArgs_hip_navatala_cfd_k_omega_s_s_t_blending
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_k_omega_s_s_t_nut_compute[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_k_omega_s_s_t_nut_compute = {
  1,
  "navatala_cfd_k_omega_s_s_t_nut_compute",
  "hip",
  "navatala_cfd_k_omega_s_s_t_nut_compute",
  "kernel:hip:navatala_cfd_k_omega_s_s_t_nut_compute",
  "abi-r1:hip:navatala_cfd_k_omega_s_s_t_nut_compute",
  "abi-r1:hip:navatala_cfd_k_omega_s_s_t_nut_compute",
  7,
  kAbiArgs_hip_navatala_cfd_k_omega_s_s_t_nut_compute
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_k_omega_s_s_t_sources[] = {
  { "k", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "omega", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "nut", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "nu", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_k_omega_s_s_t_sources = {
  1,
  "navatala_cfd_k_omega_s_s_t_sources",
  "hip",
  "navatala_cfd_k_omega_s_s_t_sources",
  "kernel:hip:navatala_cfd_k_omega_s_s_t_sources",
  "abi-r1:hip:navatala_cfd_k_omega_s_s_t_sources",
  "abi-r1:hip:navatala_cfd_k_omega_s_s_t_sources",
  21,
  kAbiArgs_hip_navatala_cfd_k_omega_s_s_t_sources
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mu_eff_from_rho_nu_eff[] = {
  { "rho", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mu_eff_from_rho_nu_eff = {
  1,
  "navatala_cfd_mu_eff_from_rho_nu_eff",
  "hip",
  "navatala_cfd_mu_eff_from_rho_nu_eff",
  "kernel:hip:navatala_cfd_mu_eff_from_rho_nu_eff",
  "abi-r1:hip:navatala_cfd_mu_eff_from_rho_nu_eff",
  "abi-r1:hip:navatala_cfd_mu_eff_from_rho_nu_eff",
  4,
  kAbiArgs_hip_navatala_cfd_mu_eff_from_rho_nu_eff
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_mu_eff_from_rho_nut_and_nu_mix[] = {
  { "rho", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_mu_eff_from_rho_nut_and_nu_mix = {
  1,
  "navatala_cfd_mu_eff_from_rho_nut_and_nu_mix",
  "hip",
  "navatala_cfd_mu_eff_from_rho_nut_and_nu_mix",
  "kernel:hip:navatala_cfd_mu_eff_from_rho_nut_and_nu_mix",
  "abi-r1:hip:navatala_cfd_mu_eff_from_rho_nut_and_nu_mix",
  "abi-r1:hip:navatala_cfd_mu_eff_from_rho_nut_and_nu_mix",
  5,
  kAbiArgs_hip_navatala_cfd_mu_eff_from_rho_nut_and_nu_mix
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_turbulence_add_explicit_source_device[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_turbulence_add_explicit_source_device = {
  1,
  "navatala_cfd_turbulence_add_explicit_source_device",
  "hip",
  "navatala_cfd_turbulence_add_explicit_source_device",
  "kernel:hip:navatala_cfd_turbulence_add_explicit_source_device",
  "abi-r1:hip:navatala_cfd_turbulence_add_explicit_source_device",
  "abi-r1:hip:navatala_cfd_turbulence_add_explicit_source_device",
  4,
  kAbiArgs_hip_navatala_cfd_turbulence_add_explicit_source_device
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_turbulence_dirichlet_cell_source[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_turbulence_dirichlet_cell_source = {
  1,
  "navatala_cfd_turbulence_dirichlet_cell_source",
  "hip",
  "navatala_cfd_turbulence_dirichlet_cell_source",
  "kernel:hip:navatala_cfd_turbulence_dirichlet_cell_source",
  "abi-r1:hip:navatala_cfd_turbulence_dirichlet_cell_source",
  "abi-r1:hip:navatala_cfd_turbulence_dirichlet_cell_source",
  5,
  kAbiArgs_hip_navatala_cfd_turbulence_dirichlet_cell_source
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_turbulence_dirichlet_face_elimination[] = {
  { "owner", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "neighbour", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_turbulence_dirichlet_face_elimination = {
  1,
  "navatala_cfd_turbulence_dirichlet_face_elimination",
  "hip",
  "navatala_cfd_turbulence_dirichlet_face_elimination",
  "kernel:hip:navatala_cfd_turbulence_dirichlet_face_elimination",
  "abi-r1:hip:navatala_cfd_turbulence_dirichlet_face_elimination",
  "abi-r1:hip:navatala_cfd_turbulence_dirichlet_face_elimination",
  8,
  kAbiArgs_hip_navatala_cfd_turbulence_dirichlet_face_elimination
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_two_mag_sqr_symm_grad_u[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_two_mag_sqr_symm_grad_u = {
  1,
  "navatala_cfd_two_mag_sqr_symm_grad_u",
  "hip",
  "navatala_cfd_two_mag_sqr_symm_grad_u",
  "kernel:hip:navatala_cfd_two_mag_sqr_symm_grad_u",
  "abi-r1:hip:navatala_cfd_two_mag_sqr_symm_grad_u",
  "abi-r1:hip:navatala_cfd_two_mag_sqr_symm_grad_u",
  12,
  kAbiArgs_hip_navatala_cfd_two_mag_sqr_symm_grad_u
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_attention_row_softmax[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_attention_row_softmax = {
  1,
  "navatala_cfd_attention_row_softmax",
  "hip",
  "navatala_cfd_attention_row_softmax",
  "kernel:hip:navatala_cfd_attention_row_softmax",
  "abi-r1:hip:navatala_cfd_attention_row_softmax",
  "abi-r1:hip:navatala_cfd_attention_row_softmax",
  4,
  kAbiArgs_hip_navatala_cfd_attention_row_softmax
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_attention_value_projection[] = {
  { "weights", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_attention_value_projection = {
  1,
  "navatala_cfd_attention_value_projection",
  "hip",
  "navatala_cfd_attention_value_projection",
  "kernel:hip:navatala_cfd_attention_value_projection",
  "abi-r1:hip:navatala_cfd_attention_value_projection",
  "abi-r1:hip:navatala_cfd_attention_value_projection",
  6,
  kAbiArgs_hip_navatala_cfd_attention_value_projection
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_gather_mean[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_gather_mean = {
  1,
  "navatala_cfd_gather_mean",
  "hip",
  "navatala_cfd_gather_mean",
  "kernel:hip:navatala_cfd_gather_mean",
  "abi-r1:hip:navatala_cfd_gather_mean",
  "abi-r1:hip:navatala_cfd_gather_mean",
  6,
  kAbiArgs_hip_navatala_cfd_gather_mean
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_layer_norm_forward[] = {
  { "x", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_layer_norm_forward = {
  1,
  "navatala_cfd_layer_norm_forward",
  "hip",
  "navatala_cfd_layer_norm_forward",
  "kernel:hip:navatala_cfd_layer_norm_forward",
  "abi-r1:hip:navatala_cfd_layer_norm_forward",
  "abi-r1:hip:navatala_cfd_layer_norm_forward",
  5,
  kAbiArgs_hip_navatala_cfd_layer_norm_forward
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_radius_count[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_radius_count = {
  1,
  "navatala_cfd_radius_count",
  "hip",
  "navatala_cfd_radius_count",
  "kernel:hip:navatala_cfd_radius_count",
  "abi-r1:hip:navatala_cfd_radius_count",
  "abi-r1:hip:navatala_cfd_radius_count",
  6,
  kAbiArgs_hip_navatala_cfd_radius_count
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_scaled_dot_product[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_scaled_dot_product = {
  1,
  "navatala_cfd_scaled_dot_product",
  "hip",
  "navatala_cfd_scaled_dot_product",
  "kernel:hip:navatala_cfd_scaled_dot_product",
  "abi-r1:hip:navatala_cfd_scaled_dot_product",
  "abi-r1:hip:navatala_cfd_scaled_dot_product",
  6,
  kAbiArgs_hip_navatala_cfd_scaled_dot_product
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_scatter_add_atomic[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_scatter_add_atomic = {
  1,
  "navatala_cfd_scatter_add_atomic",
  "hip",
  "navatala_cfd_scatter_add_atomic",
  "kernel:hip:navatala_cfd_scatter_add_atomic",
  "abi-r1:hip:navatala_cfd_scatter_add_atomic",
  "abi-r1:hip:navatala_cfd_scatter_add_atomic",
  5,
  kAbiArgs_hip_navatala_cfd_scatter_add_atomic
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_spectral_complex_mul[] = {
  { "x", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_spectral_complex_mul = {
  1,
  "navatala_cfd_spectral_complex_mul",
  "hip",
  "navatala_cfd_spectral_complex_mul",
  "kernel:hip:navatala_cfd_spectral_complex_mul",
  "abi-r1:hip:navatala_cfd_spectral_complex_mul",
  "abi-r1:hip:navatala_cfd_spectral_complex_mul",
  4,
  kAbiArgs_hip_navatala_cfd_spectral_complex_mul
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_spectral_conv1d_mode_sliced[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_spectral_conv1d_mode_sliced = {
  1,
  "navatala_cfd_spectral_conv1d_mode_sliced",
  "hip",
  "navatala_cfd_spectral_conv1d_mode_sliced",
  "kernel:hip:navatala_cfd_spectral_conv1d_mode_sliced",
  "abi-r1:hip:navatala_cfd_spectral_conv1d_mode_sliced",
  "abi-r1:hip:navatala_cfd_spectral_conv1d_mode_sliced",
  6,
  kAbiArgs_hip_navatala_cfd_spectral_conv1d_mode_sliced
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_spectral_conv2d_mode_sliced[] = {
  { "xFt", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "wPos", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "wNeg", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "numBatches", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 4, 4, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_spectral_conv2d_mode_sliced = {
  1,
  "navatala_cfd_spectral_conv2d_mode_sliced",
  "hip",
  "navatala_cfd_spectral_conv2d_mode_sliced",
  "kernel:hip:navatala_cfd_spectral_conv2d_mode_sliced",
  "abi-r1:hip:navatala_cfd_spectral_conv2d_mode_sliced",
  "abi-r1:hip:navatala_cfd_spectral_conv2d_mode_sliced",
  7,
  kAbiArgs_hip_navatala_cfd_spectral_conv2d_mode_sliced
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_spectral_conv3d_mode_sliced[] = {
  { "xFt", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_spectral_conv3d_mode_sliced = {
  1,
  "navatala_cfd_spectral_conv3d_mode_sliced",
  "hip",
  "navatala_cfd_spectral_conv3d_mode_sliced",
  "kernel:hip:navatala_cfd_spectral_conv3d_mode_sliced",
  "abi-r1:hip:navatala_cfd_spectral_conv3d_mode_sliced",
  "abi-r1:hip:navatala_cfd_spectral_conv3d_mode_sliced",
  10,
  kAbiArgs_hip_navatala_cfd_spectral_conv3d_mode_sliced
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_spectral_conv4d_mode_sliced[] = {
  { "xFt", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_spectral_conv4d_mode_sliced = {
  1,
  "navatala_cfd_spectral_conv4d_mode_sliced",
  "hip",
  "navatala_cfd_spectral_conv4d_mode_sliced",
  "kernel:hip:navatala_cfd_spectral_conv4d_mode_sliced",
  "abi-r1:hip:navatala_cfd_spectral_conv4d_mode_sliced",
  "abi-r1:hip:navatala_cfd_spectral_conv4d_mode_sliced",
  15,
  kAbiArgs_hip_navatala_cfd_spectral_conv4d_mode_sliced
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_compute_face_area_pair_weights_f32[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_compute_face_area_pair_weights_f32 = {
  1,
  "navatala_cfd_compute_face_area_pair_weights_f32",
  "hip",
  "navatala_cfd_compute_face_area_pair_weights_f32",
  "kernel:hip:navatala_cfd_compute_face_area_pair_weights_f32",
  "abi-r1:hip:navatala_cfd_compute_face_area_pair_weights_f32",
  "abi-r1:hip:navatala_cfd_compute_face_area_pair_weights_f32",
  6,
  kAbiArgs_hip_navatala_cfd_compute_face_area_pair_weights_f32
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_restrict_by_agglomeration_f32[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_restrict_by_agglomeration_f32 = {
  1,
  "navatala_cfd_restrict_by_agglomeration_f32",
  "hip",
  "navatala_cfd_restrict_by_agglomeration_f32",
  "kernel:hip:navatala_cfd_restrict_by_agglomeration_f32",
  "abi-r1:hip:navatala_cfd_restrict_by_agglomeration_f32",
  "abi-r1:hip:navatala_cfd_restrict_by_agglomeration_f32",
  4,
  kAbiArgs_hip_navatala_cfd_restrict_by_agglomeration_f32
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_restrict_by_agglomeration_gather_f32[] = {
  { "fineField", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_restrict_by_agglomeration_gather_f32 = {
  1,
  "navatala_cfd_restrict_by_agglomeration_gather_f32",
  "hip",
  "navatala_cfd_restrict_by_agglomeration_gather_f32",
  "kernel:hip:navatala_cfd_restrict_by_agglomeration_gather_f32",
  "abi-r1:hip:navatala_cfd_restrict_by_agglomeration_gather_f32",
  "abi-r1:hip:navatala_cfd_restrict_by_agglomeration_gather_f32",
  5,
  kAbiArgs_hip_navatala_cfd_restrict_by_agglomeration_gather_f32
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_prolongate_by_agglomeration_f32[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_prolongate_by_agglomeration_f32 = {
  1,
  "navatala_cfd_prolongate_by_agglomeration_f32",
  "hip",
  "navatala_cfd_prolongate_by_agglomeration_f32",
  "kernel:hip:navatala_cfd_prolongate_by_agglomeration_f32",
  "abi-r1:hip:navatala_cfd_prolongate_by_agglomeration_f32",
  "abi-r1:hip:navatala_cfd_prolongate_by_agglomeration_f32",
  4,
  kAbiArgs_hip_navatala_cfd_prolongate_by_agglomeration_f32
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_diag_inv_apply_f32[] = {
  { "r", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "diagInv", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "n", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 4, 4, 256, nullptr, 0, 0 },
  { "z", 3, KernelArgumentRole::Output, KernelAccessMode::WriteOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 }
};
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_diag_inv_apply_f32 = {
  1,
  "navatala_cfd_diag_inv_apply_f32",
  "hip",
  "navatala_cfd_diag_inv_apply_f32",
  "kernel:hip:navatala_cfd_diag_inv_apply_f32",
  "abi-r1:hip:navatala_cfd_diag_inv_apply_f32",
  "abi-r1:hip:navatala_cfd_diag_inv_apply_f32",
  4,
  kAbiArgs_hip_navatala_cfd_diag_inv_apply_f32
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_csr_sp_m_v_mixed_f32_f64[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_csr_sp_m_v_mixed_f32_f64 = {
  1,
  "navatala_cfd_csr_sp_m_v_mixed_f32_f64",
  "hip",
  "navatala_cfd_csr_sp_m_v_mixed_f32_f64",
  "kernel:hip:navatala_cfd_csr_sp_m_v_mixed_f32_f64",
  "abi-r1:hip:navatala_cfd_csr_sp_m_v_mixed_f32_f64",
  "abi-r1:hip:navatala_cfd_csr_sp_m_v_mixed_f32_f64",
  6,
  kAbiArgs_hip_navatala_cfd_csr_sp_m_v_mixed_f32_f64
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_chebyshev_sweep_mixed_f32_f64[] = {
  { "r", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_chebyshev_sweep_mixed_f32_f64 = {
  1,
  "navatala_cfd_chebyshev_sweep_mixed_f32_f64",
  "hip",
  "navatala_cfd_chebyshev_sweep_mixed_f32_f64",
  "kernel:hip:navatala_cfd_chebyshev_sweep_mixed_f32_f64",
  "abi-r1:hip:navatala_cfd_chebyshev_sweep_mixed_f32_f64",
  "abi-r1:hip:navatala_cfd_chebyshev_sweep_mixed_f32_f64",
  7,
  kAbiArgs_hip_navatala_cfd_chebyshev_sweep_mixed_f32_f64
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_ell_sp_m_v_mixed_f32_f64[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_ell_sp_m_v_mixed_f32_f64 = {
  1,
  "navatala_cfd_ell_sp_m_v_mixed_f32_f64",
  "hip",
  "navatala_cfd_ell_sp_m_v_mixed_f32_f64",
  "kernel:hip:navatala_cfd_ell_sp_m_v_mixed_f32_f64",
  "abi-r1:hip:navatala_cfd_ell_sp_m_v_mixed_f32_f64",
  "abi-r1:hip:navatala_cfd_ell_sp_m_v_mixed_f32_f64",
  6,
  kAbiArgs_hip_navatala_cfd_ell_sp_m_v_mixed_f32_f64
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_chebyshev_smooth_post_fused_mixed[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_chebyshev_smooth_post_fused_mixed = {
  1,
  "navatala_cfd_chebyshev_smooth_post_fused_mixed",
  "hip",
  "navatala_cfd_chebyshev_smooth_post_fused_mixed",
  "kernel:hip:navatala_cfd_chebyshev_smooth_post_fused_mixed",
  "abi-r1:hip:navatala_cfd_chebyshev_smooth_post_fused_mixed",
  "abi-r1:hip:navatala_cfd_chebyshev_smooth_post_fused_mixed",
  9,
  kAbiArgs_hip_navatala_cfd_chebyshev_smooth_post_fused_mixed
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_gamg_proc_pack_f32[] = {
  { "x", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_gamg_proc_pack_f32 = {
  1,
  "navatala_cfd_gamg_proc_pack_f32",
  "hip",
  "navatala_cfd_gamg_proc_pack_f32",
  "kernel:hip:navatala_cfd_gamg_proc_pack_f32",
  "abi-r1:hip:navatala_cfd_gamg_proc_pack_f32",
  "abi-r1:hip:navatala_cfd_gamg_proc_pack_f32",
  4,
  kAbiArgs_hip_navatala_cfd_gamg_proc_pack_f32
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_gamg_proc_add_f32[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_gamg_proc_add_f32 = {
  1,
  "navatala_cfd_gamg_proc_add_f32",
  "hip",
  "navatala_cfd_gamg_proc_add_f32",
  "kernel:hip:navatala_cfd_gamg_proc_add_f32",
  "abi-r1:hip:navatala_cfd_gamg_proc_add_f32",
  "abi-r1:hip:navatala_cfd_gamg_proc_add_f32",
  5,
  kAbiArgs_hip_navatala_cfd_gamg_proc_add_f32
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_gamg_proc_add_gather_f32[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_gamg_proc_add_gather_f32 = {
  1,
  "navatala_cfd_gamg_proc_add_gather_f32",
  "hip",
  "navatala_cfd_gamg_proc_add_gather_f32",
  "kernel:hip:navatala_cfd_gamg_proc_add_gather_f32",
  "abi-r1:hip:navatala_cfd_gamg_proc_add_gather_f32",
  "abi-r1:hip:navatala_cfd_gamg_proc_add_gather_f32",
  6,
  kAbiArgs_hip_navatala_cfd_gamg_proc_add_gather_f32
};

bool tryGetKernelAbiManifest_hip_cfd(const std::string& backend, const std::string& kernelName, const KernelAbiManifestInfo*& out) {
  if (backend == "hip" && kernelName == "navatala_cfd_boundary_force_partials") {
    out = &kAbiManifest_hip_navatala_cfd_boundary_force_partials;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_alpha_proc_face_interp") {
    out = &kAbiManifest_hip_navatala_cfd_alpha_proc_face_interp;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mules_proc_face_update") {
    out = &kAbiManifest_hip_navatala_cfd_mules_proc_face_update;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mules_proc_face_update_f64") {
    out = &kAbiManifest_hip_navatala_cfd_mules_proc_face_update_f64;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pack_owner_values") {
    out = &kAbiManifest_hip_navatala_cfd_pack_owner_values;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pack_owner_values_f64") {
    out = &kAbiManifest_hip_navatala_cfd_pack_owner_values_f64;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scatter_values_and_mask") {
    out = &kAbiManifest_hip_navatala_cfd_scatter_values_and_mask;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pack_mu_grad_owner") {
    out = &kAbiManifest_hip_navatala_cfd_pack_mu_grad_owner;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pack_owner_rho_vec3") {
    out = &kAbiManifest_hip_navatala_cfd_pack_owner_rho_vec3;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pack_owner_vec3") {
    out = &kAbiManifest_hip_navatala_cfd_pack_owner_vec3;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pack_owner_vec3_mu") {
    out = &kAbiManifest_hip_navatala_cfd_pack_owner_vec3_mu;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scatter_mu_grad_ghost") {
    out = &kAbiManifest_hip_navatala_cfd_scatter_mu_grad_ghost;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scatter_rho_vec3_and_mask") {
    out = &kAbiManifest_hip_navatala_cfd_scatter_rho_vec3_and_mask;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scatter_vec3_and_mask") {
    out = &kAbiManifest_hip_navatala_cfd_scatter_vec3_and_mask;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scatter_vec3_mu_and_mask") {
  ...
  if (backend == "hip" && kernelName == "navatala_cfd_div_rho_phi_u_explicit") {
    out = &kAbiManifest_hip_navatala_cfd_div_rho_phi_u_explicit;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_hby_a_from_u_and_terms") {
    out = &kAbiManifest_hip_navatala_cfd_hby_a_from_u_and_terms;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_apply_sp_to_hby_a") {
    out = &kAbiManifest_hip_navatala_cfd_apply_sp_to_hby_a;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_laplacian_scalar_explicit") {
    out = &kAbiManifest_hip_navatala_cfd_laplacian_scalar_explicit;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_laplacian_vector_explicit") {
    out = &kAbiManifest_hip_navatala_cfd_laplacian_vector_explicit;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_predictor_hby_a_from_source") {
    out = &kAbiManifest_hip_navatala_cfd_predictor_hby_a_from_source;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_predictor_inverse_diag") {
    out = &kAbiManifest_hip_navatala_cfd_predictor_inverse_diag;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_predictor_phi_hby_a_fuse") {
    out = &kAbiManifest_hip_navatala_cfd_predictor_phi_hby_a_fuse;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_axpy_cells") {
    out = &kAbiManifest_hip_navatala_cfd_axpy_cells;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_ddt_diagonal") {
    out = &kAbiManifest_hip_navatala_cfd_ddt_diagonal;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_ddt_source") {
    out = &kAbiManifest_hip_navatala_cfd_ddt_source;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_div_upwind_coeffs") {
    out = &kAbiManifest_hip_navatala_cfd_div_upwind_coeffs;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_fvm_sp") {
    out = &kAbiManifest_hip_navatala_cfd_fvm_sp;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_fvm_su_sp") {
    out = &kAbiManifest_hip_navatala_cfd_fvm_su_sp;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_laplacian_coeffs") {
    out = &kAbiManifest_hip_navatala_cfd_laplacian_coeffs;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_laplacian_coeffs_accum") {
    out = &kAbiManifest_hip_navatala_cfd_laplacian_coeffs_accum;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_linear_upwind_correction_source") {
    out = &kAbiManifest_hip_navatala_cfd_linear_upwind_correction_source;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_matrix_relax") {
    out = &kAbiManifest_hip_navatala_cfd_matrix_relax;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_neg_sum_diag") {
    out = &kAbiManifest_hip_navatala_cfd_neg_sum_diag;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pack_boundary_adjacent_cells") {
    out = &kAbiManifest_hip_navatala_cfd_pack_boundary_adjacent_cells;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scalar_jacobi_clamp_min") {
    out = &kAbiManifest_hip_navatala_cfd_scalar_jacobi_clamp_min;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scalar_jacobi_update") {
    out = &kAbiManifest_hip_navatala_cfd_scalar_jacobi_update;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scalar_ldu_coupled_interface_add") {
    out = &kAbiManifest_hip_navatala_cfd_scalar_ldu_coupled_interface_add;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scalar_ldu_mat_vec") {
    out = &kAbiManifest_hip_navatala_cfd_scalar_ldu_mat_vec;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scalar_ldu_multi_color_gs_sweep") {
    out = &kAbiManifest_hip_navatala_cfd_scalar_ldu_multi_color_gs_sweep;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_sum_mag_off_diag") {
    out = &kAbiManifest_hip_navatala_cfd_sum_mag_off_diag;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_upwind_weights") {
    out = &kAbiManifest_hip_navatala_cfd_upwind_weights;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_axpy_in_place") {
    out = &kAbiManifest_hip_navatala_cfd_gamg_axpy_in_place;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_compute_scale_factor") {
    out = &kAbiManifest_hip_navatala_cfd_gamg_compute_scale_factor;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_dot_finalize") {
    out = &kAbiManifest_hip_navatala_cfd_gamg_dot_finalize;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_negate_scalar") {
    out = &kAbiManifest_hip_navatala_cfd_gamg_negate_scalar;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_scale_correction") {
    out = &kAbiManifest_hip_navatala_cfd_gamg_scale_correction;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_dic_build") {
    out = &kAbiManifest_hip_navatala_cfd_mg_dic_build;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_dic_smooth") {
    out = &kAbiManifest_hip_navatala_cfd_mg_dic_smooth;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_coarse_jacobi") {
    out = &kAbiManifest_hip_navatala_cfd_mg_coarse_jacobi;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_coarse_matvec") {
    out = &kAbiManifest_hip_navatala_cfd_mg_coarse_matvec;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_coarse_rhs") {
    out = &kAbiManifest_hip_navatala_cfd_mg_coarse_rhs;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_find_best_match") {
    out = &kAbiManifest_hip_navatala_cfd_mg_find_best_match;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_pair_match") {
    out = &kAbiManifest_hip_navatala_cfd_mg_pair_match;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_prolong_add") {
    out = &kAbiManifest_hip_navatala_cfd_mg_prolong_add;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_renumber_aggregates") {
    out = &kAbiManifest_hip_navatala_cfd_mg_renumber_aggregates;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_renumber_propagate") {
    out = &kAbiManifest_hip_navatala_cfd_mg_renumber_propagate;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_resolve_chains") {
    out = &kAbiManifest_hip_navatala_cfd_mg_resolve_chains;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_restrict_sum") {
    out = &kAbiManifest_hip_navatala_cfd_mg_restrict_sum;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_strength_of_connection") {
    out = &kAbiManifest_hip_navatala_cfd_mg_strength_of_connection;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_zero_coarse") {
    out = &kAbiManifest_hip_navatala_cfd_mg_zero_coarse;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_phi_correct") {
    out = &kAbiManifest_hip_navatala_cfd_phi_correct;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_phi_correct_boundary") {
    out = &kAbiManifest_hip_navatala_cfd_phi_correct_boundary;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_phi_correct_internal") {
    out = &kAbiManifest_hip_navatala_cfd_phi_correct_internal;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_axpy_in_place") {
    out = &kAbiManifest_hip_navatala_cfd_axpy_in_place;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_bc_dirichlet_face_flux") {
    out = &kAbiManifest_hip_navatala_cfd_bc_dirichlet_face_flux;
    return true;
  }
  ...
  if (backend == "hip" && kernelName == "navatala_cfd_coeff_to_cf_in_place") {
    out = &kAbiManifest_hip_navatala_cfd_coeff_to_cf_in_place;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_diag_from_cf") {
    out = &kAbiManifest_hip_navatala_cfd_diag_from_cf;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_dic_apply") {
    out = &kAbiManifest_hip_navatala_cfd_dic_apply;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_dic_build_reciprocal_d") {
    out = &kAbiManifest_hip_navatala_cfd_dic_build_reciprocal_d;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_dot_partials") {
    out = &kAbiManifest_hip_navatala_cfd_dot_partials;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_face_flux") {
    out = &kAbiManifest_hip_navatala_cfd_face_flux;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_inv_diag") {
    out = &kAbiManifest_hip_navatala_cfd_inv_diag;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_laplacian_from_face_flux") {
    out = &kAbiManifest_hip_navatala_cfd_laplacian_from_face_flux;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mul_by_vol_in_place") {
    out = &kAbiManifest_hip_navatala_cfd_mul_by_vol_in_place;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_ref_add_ax") {
    out = &kAbiManifest_hip_navatala_cfd_ref_add_ax;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_ref_add_ax_native_f64") {
    out = &kAbiManifest_hip_navatala_cfd_ref_add_ax_native_f64;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_ref_add_b") {
    out = &kAbiManifest_hip_navatala_cfd_ref_add_b;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_ref_add_b_native_f64") {
    out = &kAbiManifest_hip_navatala_cfd_ref_add_b_native_f64;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_rhs_sub") {
    out = &kAbiManifest_hip_navatala_cfd_rhs_sub;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_shift_in_place") {
    out = &kAbiManifest_hip_navatala_cfd_shift_in_place;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_sum_abs_partials") {
    out = &kAbiManifest_hip_navatala_cfd_sum_abs_partials;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_upper_from_cf") {
    out = &kAbiManifest_hip_navatala_cfd_upper_from_cf;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_xpay_in_place") {
    out = &kAbiManifest_hip_navatala_cfd_xpay_in_place;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_dot_finalize_partials") {
    out = &kAbiManifest_hip_navatala_cfd_dot_finalize_partials;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_negate_scalar") {
    out = &kAbiManifest_hip_navatala_cfd_negate_scalar;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pcg_compute_alpha") {
    out = &kAbiManifest_hip_navatala_cfd_pcg_compute_alpha;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pcg_compute_beta") {
    out = &kAbiManifest_hip_navatala_cfd_pcg_compute_beta;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_exact_mat_vec") {
    out = &kAbiManifest_hip_navatala_cfd_exact_mat_vec;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_exact_mat_vec_f64") {
    out = &kAbiManifest_hip_navatala_cfd_exact_mat_vec_f64;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_exact_mat_vec_native_f64") {
    out = &kAbiManifest_hip_navatala_cfd_exact_mat_vec_native_f64;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_face_flux_boundary") {
    out = &kAbiManifest_hip_navatala_cfd_face_flux_boundary;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_face_flux_internal") {
    out = &kAbiManifest_hip_navatala_cfd_face_flux_internal;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pressure_face_flux_correction_corrected") {
    out = &kAbiManifest_hip_navatala_cfd_pressure_face_flux_correction_corrected;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pressure_sn_grad") {
    out = &kAbiManifest_hip_navatala_cfd_pressure_sn_grad;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_u_correct") {
    out = &kAbiManifest_hip_navatala_cfd_u_correct;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_u_correct_reconstruct") {
    out = &kAbiManifest_hip_navatala_cfd_u_correct_reconstruct;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_grad_alpha_cell") {
    out = &kAbiManifest_hip_navatala_cfd_grad_alpha_cell;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_grad_alpha_cell_f64") {
    out = &kAbiManifest_hip_navatala_cfd_grad_alpha_cell_f64;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_n_hatf_all") {
    out = &kAbiManifest_hip_navatala_cfd_n_hatf_all;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_n_hatf_all_f64") {
    out = &kAbiManifest_hip_navatala_cfd_n_hatf_all_f64;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_stf_all_faces") {
    out = &kAbiManifest_hip_navatala_cfd_stf_all_faces;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_stf_all_faces_f64") {
    out = &kAbiManifest_hip_navatala_cfd_stf_all_faces_f64;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_k_omega_s_s_t_blending") {
    out = &kAbiManifest_hip_navatala_cfd_k_omega_s_s_t_blending;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_k_omega_s_s_t_nut_compute") {
    out = &kAbiManifest_hip_navatala_cfd_k_omega_s_s_t_nut_compute;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_k_omega_s_s_t_sources") {
    out = &kAbiManifest_hip_navatala_cfd_k_omega_s_s_t_sources;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mu_eff_from_rho_nu_eff") {
    out = &kAbiManifest_hip_navatala_cfd_mu_eff_from_rho_nu_eff;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mu_eff_from_rho_nut_and_nu_mix") {
    out = &kAbiManifest_hip_navatala_cfd_mu_eff_from_rho_nut_and_nu_mix;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_turbulence_add_explicit_source_device") {
    out = &kAbiManifest_hip_navatala_cfd_turbulence_add_explicit_source_device;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_turbulence_dirichlet_cell_source") {
    out = &kAbiManifest_hip_navatala_cfd_turbulence_dirichlet_cell_source;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_turbulence_dirichlet_face_elimination") {
    out = &kAbiManifest_hip_navatala_cfd_turbulence_dirichlet_face_elimination;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_two_mag_sqr_symm_grad_u") {
    out = &kAbiManifest_hip_navatala_cfd_two_mag_sqr_symm_grad_u;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_attention_row_softmax") {
    out = &kAbiManifest_hip_navatala_cfd_attention_row_softmax;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_attention_value_projection") {
    out = &kAbiManifest_hip_navatala_cfd_attention_value_projection;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gather_mean") {
    out = &kAbiManifest_hip_navatala_cfd_gather_mean;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_layer_norm_forward") {
    out = &kAbiManifest_hip_navatala_cfd_layer_norm_forward;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_radius_count") {
    out = &kAbiManifest_hip_navatala_cfd_radius_count;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scaled_dot_product") {
    out = &kAbiManifest_hip_navatala_cfd_scaled_dot_product;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scatter_add_atomic") {
    out = &kAbiManifest_hip_navatala_cfd_scatter_add_atomic;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_spectral_complex_mul") {
    out = &kAbiManifest_hip_navatala_cfd_spectral_complex_mul;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_spectral_conv1d_mode_sliced") {
    out = &kAbiManifest_hip_navatala_cfd_spectral_conv1d_mode_sliced;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_spectral_conv2d_mode_sliced") {
    out = &kAbiManifest_hip_navatala_cfd_spectral_conv2d_mode_sliced;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_spectral_conv3d_mode_sliced") {
    out = &kAbiManifest_hip_navatala_cfd_spectral_conv3d_mode_sliced;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_spectral_conv4d_mode_sliced") {
    out = &kAbiManifest_hip_navatala_cfd_spectral_conv4d_mode_sliced;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_compute_face_area_pair_weights_f32") {
    out = &kAbiManifest_hip_navatala_cfd_compute_face_area_pair_weights_f32;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_restrict_by_agglomeration_f32") {
    out = &kAbiManifest_hip_navatala_cfd_restrict_by_agglomeration_f32;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_restrict_by_agglomeration_gather_f32") {
    out = &kAbiManifest_hip_navatala_cfd_restrict_by_agglomeration_gather_f32;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_prolongate_by_agglomeration_f32") {
    out = &kAbiManifest_hip_navatala_cfd_prolongate_by_agglomeration_f32;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_diag_inv_apply_f32") {
    out = &kAbiManifest_hip_navatala_cfd_diag_inv_apply_f32;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_csr_sp_m_v_mixed_f32_f64") {
    out = &kAbiManifest_hip_navatala_cfd_csr_sp_m_v_mixed_f32_f64;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_chebyshev_sweep_mixed_f32_f64") {
    out = &kAbiManifest_hip_navatala_cfd_chebyshev_sweep_mixed_f32_f64;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_ell_sp_m_v_mixed_f32_f64") {
    out = &kAbiManifest_hip_navatala_cfd_ell_sp_m_v_mixed_f32_f64;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_chebyshev_smooth_post_fused_mixed") {
    out = &kAbiManifest_hip_navatala_cfd_chebyshev_smooth_post_fused_mixed;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_proc_pack_f32") {
    out = &kAbiManifest_hip_navatala_cfd_gamg_proc_pack_f32;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_proc_add_f32") {
    out = &kAbiManifest_hip_navatala_cfd_gamg_proc_add_f32;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_proc_add_gather_f32") {
    out = &kAbiManifest_hip_navatala_cfd_gamg_proc_add_gather_f32;
    return true;
  }
  out = nullptr;
  return false;
...
bool tryGetKernelSource_hip_cfd(const std::string& backend, const std::string& kernelName, GpuRuntime::ProgramSource& out) {
  if (backend == "hip" && kernelName == "navatala_cfd_boundary_force_partials") {
    out.kind = GpuRuntime::ProgramSource::Kind::HipCpp;
    out.entryPoint = "navatala_cfd_boundary_force_partials";
    std::string_view sv(k_hip_navatala_cfd_boundary_force_partials);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_alpha_proc_face_interp") {
    out.kind = GpuRuntime::ProgramSource::Kind::HipCpp;
    out.entryPoint = "navatala_cfd_alpha_proc_face_interp";
    std::string_view sv(k_hip_navatala_cfd_alpha_proc_face_interp);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mules_proc_face_update") {
    out.kind = GpuRuntime::ProgramSource::Kind::HipCpp;
    out.entryPoint = "navatala_cfd_mules_proc_face_update";
    std::string_view sv(k_hip_navatala_cfd_mules_proc_face_update);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mules_proc_face_update_f64") {
    out.kind = GpuRuntime::ProgramSource::Kind::HipCpp;
    out.entryPoint = "navatala_cfd_mules_proc_face_update_f64";
    std::string_view sv(k_hip_navatala_cfd_mules_proc_face_update_f64);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pack_owner_values") {
    ...
    std::string_view sv(k_hip_navatala_cfd_pack_owner_values);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pack_owner_values_f64") {
    out.kind = GpuRuntime::ProgramSource::Kind::HipCpp;
    out.entryPoint = "navatala_cfd_pack_owner_values_f64";
    std::string_view sv(k_hip_navatala_cfd_pack_owner_values_f64);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scatter_values_and_mask") {
    ...
    std::string_view sv(k_hip_navatala_cfd_scatter_values_and_mask);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pack_mu_grad_owner") {
    ...
    std::string_view sv(k_hip_navatala_cfd_pack_mu_grad_owner);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pack_owner_rho_vec3") {
    ...
    std::string_view sv(k_hip_navatala_cfd_pack_owner_rho_vec3);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pack_owner_vec3") {
    ...
    std::string_view sv(k_hip_navatala_cfd_pack_owner_vec3);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pack_owner_vec3_mu") {
    ...
    std::string_view sv(k_hip_navatala_cfd_pack_owner_vec3_mu);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scatter_mu_grad_ghost") {
    ...
    std::string_view sv(k_hip_navatala_cfd_scatter_mu_grad_ghost);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scatter_rho_vec3_and_mask") {
    ...
    std::string_view sv(k_hip_navatala_cfd_scatter_rho_vec3_and_mask);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scatter_vec3_and_mask") {
    ...
    std::string_view sv(k_hip_navatala_cfd_scatter_vec3_and_mask);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scatter_vec3_mu_and_mask") {
    ...
    std::string_view sv(k_hip_navatala_cfd_scatter_vec3_mu_and_mask);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_cf_mesh_volume_optimizer_gradient_f32") {
  ...
  if (backend == "hip" && kernelName == "navatala_cfd_add_vol_vector") {
    ...
    std::string_view sv(k_hip_navatala_cfd_add_vol_vector);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_div_rho_phi_u_explicit") {
    ...
    std::string_view sv(k_hip_navatala_cfd_div_rho_phi_u_explicit);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_hby_a_from_u_and_terms") {
    ...
    std::string_view sv(k_hip_navatala_cfd_hby_a_from_u_and_terms);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_apply_sp_to_hby_a") {
    ...
    std::string_view sv(k_hip_navatala_cfd_apply_sp_to_hby_a);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_laplacian_scalar_explicit") {
    ...
    std::string_view sv(k_hip_navatala_cfd_laplacian_scalar_explicit);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_laplacian_vector_explicit") {
    ...
    std::string_view sv(k_hip_navatala_cfd_laplacian_vector_explicit);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_predictor_hby_a_from_source") {
    ...
    std::string_view sv(k_hip_navatala_cfd_predictor_hby_a_from_source);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_predictor_inverse_diag") {
    ...
    std::string_view sv(k_hip_navatala_cfd_predictor_inverse_diag);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_predictor_phi_hby_a_fuse") {
    ...
    std::string_view sv(k_hip_navatala_cfd_predictor_phi_hby_a_fuse);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_axpy_cells") {
    ...
    std::string_view sv(k_hip_navatala_cfd_axpy_cells);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_ddt_diagonal") {
    ...
    std::string_view sv(k_hip_navatala_cfd_ddt_diagonal);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_ddt_source") {
    out.kind = GpuRuntime::ProgramSource::Kind::HipCpp;
    out.entryPoint = "navatala_cfd_ddt_source";
    std::string_view sv(k_hip_navatala_cfd_ddt_source);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_div_upwind_coeffs") {
    ...
    std::string_view sv(k_hip_navatala_cfd_div_upwind_coeffs);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_fvm_sp") {
    ...
    std::string_view sv(k_hip_navatala_cfd_fvm_sp);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_fvm_su_sp") {
    ...
    std::string_view sv(k_hip_navatala_cfd_fvm_su_sp);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_laplacian_coeffs") {
    ...
    std::string_view sv(k_hip_navatala_cfd_laplacian_coeffs);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_laplacian_coeffs_accum") {
    ...
    std::string_view sv(k_hip_navatala_cfd_laplacian_coeffs_accum);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_linear_upwind_correction_source") {
    ...
    std::string_view sv(k_hip_navatala_cfd_linear_upwind_correction_source);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_matrix_relax") {
    ...
    std::string_view sv(k_hip_navatala_cfd_matrix_relax);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_neg_sum_diag") {
    ...
    std::string_view sv(k_hip_navatala_cfd_neg_sum_diag);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pack_boundary_adjacent_cells") {
    ...
    std::string_view sv(k_hip_navatala_cfd_pack_boundary_adjacent_cells);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scalar_jacobi_clamp_min") {
    ...
    std::string_view sv(k_hip_navatala_cfd_scalar_jacobi_clamp_min);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scalar_jacobi_update") {
    ...
    std::string_view sv(k_hip_navatala_cfd_scalar_jacobi_update);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scalar_ldu_coupled_interface_add") {
    ...
    std::string_view sv(k_hip_navatala_cfd_scalar_ldu_coupled_interface_add);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scalar_ldu_mat_vec") {
    ...
    std::string_view sv(k_hip_navatala_cfd_scalar_ldu_mat_vec);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scalar_ldu_multi_color_gs_sweep") {
    ...
    std::string_view sv(k_hip_navatala_cfd_scalar_ldu_multi_color_gs_sweep);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_sum_mag_off_diag") {
    ...
    std::string_view sv(k_hip_navatala_cfd_sum_mag_off_diag);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_upwind_weights") {
    out.kind = GpuRuntime::ProgramSource::Kind::HipCpp;
    out.entryPoint = "navatala_cfd_upwind_weights";
    std::string_view sv(k_hip_navatala_cfd_upwind_weights);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_axpy_in_place") {
    ...
    std::string_view sv(k_hip_navatala_cfd_gamg_axpy_in_place);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_compute_scale_factor") {
    ...
    std::string_view sv(k_hip_navatala_cfd_gamg_compute_scale_factor);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_dot_finalize") {
    ...
    std::string_view sv(k_hip_navatala_cfd_gamg_dot_finalize);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_negate_scalar") {
    ...
    std::string_view sv(k_hip_navatala_cfd_gamg_negate_scalar);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_scale_correction") {
    ...
    std::string_view sv(k_hip_navatala_cfd_gamg_scale_correction);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_dic_build") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mg_dic_build);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_dic_smooth") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mg_dic_smooth);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_coarse_jacobi") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mg_coarse_jacobi);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_coarse_matvec") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mg_coarse_matvec);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_coarse_rhs") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mg_coarse_rhs);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_find_best_match") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mg_find_best_match);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_pair_match") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mg_pair_match);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_prolong_add") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mg_prolong_add);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_renumber_aggregates") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mg_renumber_aggregates);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_renumber_propagate") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mg_renumber_propagate);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_resolve_chains") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mg_resolve_chains);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_restrict_sum") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mg_restrict_sum);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_strength_of_connection") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mg_strength_of_connection);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mg_zero_coarse") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mg_zero_coarse);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_phi_correct") {
    ...
    std::string_view sv(k_hip_navatala_cfd_phi_correct);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_phi_correct_boundary") {
    ...
    std::string_view sv(k_hip_navatala_cfd_phi_correct_boundary);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_phi_correct_internal") {
    ...
    std::string_view sv(k_hip_navatala_cfd_phi_correct_internal);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_axpy_in_place") {
    ...
    std::string_view sv(k_hip_navatala_cfd_axpy_in_place);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_bc_dirichlet_face_flux") {
    ...
    std::string_view sv(k_hip_navatala_cfd_bc_dirichlet_face_flux);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_bc_sn_grad_face_flux") {
  ...
  if (backend == "hip" && kernelName == "navatala_cfd_coeff_to_cf_in_place") {
    ...
    std::string_view sv(k_hip_navatala_cfd_coeff_to_cf_in_place);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_diag_from_cf") {
    ...
    std::string_view sv(k_hip_navatala_cfd_diag_from_cf);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_dic_apply") {
    ...
    std::string_view sv(k_hip_navatala_cfd_dic_apply);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_dic_build_reciprocal_d") {
    ...
    std::string_view sv(k_hip_navatala_cfd_dic_build_reciprocal_d);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_dot_partials") {
    ...
    std::string_view sv(k_hip_navatala_cfd_dot_partials);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_face_flux") {
    ...
    std::string_view sv(k_hip_navatala_cfd_face_flux);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_inv_diag") {
    ...
    std::string_view sv(k_hip_navatala_cfd_inv_diag);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_laplacian_from_face_flux") {
    ...
    std::string_view sv(k_hip_navatala_cfd_laplacian_from_face_flux);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mul_by_vol_in_place") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mul_by_vol_in_place);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_ref_add_ax") {
    ...
    std::string_view sv(k_hip_navatala_cfd_ref_add_ax);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_ref_add_ax_native_f64") {
    out.kind = GpuRuntime::ProgramSource::Kind::HipCpp;
    out.entryPoint = "navatala_cfd_ref_add_ax_native_f64";
    std::string_view sv(k_hip_navatala_cfd_ref_add_ax_native_f64);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_ref_add_b") {
    ...
    std::string_view sv(k_hip_navatala_cfd_ref_add_b);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_ref_add_b_native_f64") {
    out.kind = GpuRuntime::ProgramSource::Kind::HipCpp;
    out.entryPoint = "navatala_cfd_ref_add_b_native_f64";
    std::string_view sv(k_hip_navatala_cfd_ref_add_b_native_f64);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_rhs_sub") {
    ...
    std::string_view sv(k_hip_navatala_cfd_rhs_sub);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_shift_in_place") {
    ...
    std::string_view sv(k_hip_navatala_cfd_shift_in_place);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_sum_abs_partials") {
    ...
    std::string_view sv(k_hip_navatala_cfd_sum_abs_partials);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_upper_from_cf") {
    ...
    std::string_view sv(k_hip_navatala_cfd_upper_from_cf);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_xpay_in_place") {
    ...
    std::string_view sv(k_hip_navatala_cfd_xpay_in_place);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_dot_finalize_partials") {
    ...
    std::string_view sv(k_hip_navatala_cfd_dot_finalize_partials);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_negate_scalar") {
    ...
    std::string_view sv(k_hip_navatala_cfd_negate_scalar);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pcg_compute_alpha") {
    ...
    std::string_view sv(k_hip_navatala_cfd_pcg_compute_alpha);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pcg_compute_beta") {
    ...
    std::string_view sv(k_hip_navatala_cfd_pcg_compute_beta);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_exact_mat_vec") {
    ...
    std::string_view sv(k_hip_navatala_cfd_exact_mat_vec);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_exact_mat_vec_f64") {
    ...
    std::string_view sv(k_hip_navatala_cfd_exact_mat_vec_f64);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_exact_mat_vec_native_f64") {
    out.kind = GpuRuntime::ProgramSource::Kind::HipCpp;
    out.entryPoint = "navatala_cfd_exact_mat_vec_native_f64";
    std::string_view sv(k_hip_navatala_cfd_exact_mat_vec_native_f64);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_face_flux_boundary") {
    ...
    std::string_view sv(k_hip_navatala_cfd_face_flux_boundary);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_face_flux_internal") {
    ...
    std::string_view sv(k_hip_navatala_cfd_face_flux_internal);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pressure_face_flux_correction_corrected") {
    ...
    std::string_view sv(k_hip_navatala_cfd_pressure_face_flux_correction_corrected);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_pressure_sn_grad") {
    ...
    std::string_view sv(k_hip_navatala_cfd_pressure_sn_grad);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_u_correct") {
    ...
    std::string_view sv(k_hip_navatala_cfd_u_correct);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_u_correct_reconstruct") {
    ...
    std::string_view sv(k_hip_navatala_cfd_u_correct_reconstruct);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_grad_alpha_cell") {
    ...
    std::string_view sv(k_hip_navatala_cfd_grad_alpha_cell);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_grad_alpha_cell_f64") {
    ...
    std::string_view sv(k_hip_navatala_cfd_grad_alpha_cell_f64);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_n_hatf_all") {
    ...
    std::string_view sv(k_hip_navatala_cfd_n_hatf_all);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_n_hatf_all_f64") {
    ...
    std::string_view sv(k_hip_navatala_cfd_n_hatf_all_f64);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_stf_all_faces") {
    ...
    std::string_view sv(k_hip_navatala_cfd_stf_all_faces);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_stf_all_faces_f64") {
    ...
    std::string_view sv(k_hip_navatala_cfd_stf_all_faces_f64);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_k_omega_s_s_t_blending") {
    ...
    std::string_view sv(k_hip_navatala_cfd_k_omega_s_s_t_blending);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_k_omega_s_s_t_nut_compute") {
    ...
    std::string_view sv(k_hip_navatala_cfd_k_omega_s_s_t_nut_compute);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_k_omega_s_s_t_sources") {
    ...
    std::string_view sv(k_hip_navatala_cfd_k_omega_s_s_t_sources);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mu_eff_from_rho_nu_eff") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mu_eff_from_rho_nu_eff);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_mu_eff_from_rho_nut_and_nu_mix") {
    ...
    std::string_view sv(k_hip_navatala_cfd_mu_eff_from_rho_nut_and_nu_mix);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_turbulence_add_explicit_source_device") {
    ...
    std::string_view sv(k_hip_navatala_cfd_turbulence_add_explicit_source_device);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_turbulence_dirichlet_cell_source") {
    ...
    std::string_view sv(k_hip_navatala_cfd_turbulence_dirichlet_cell_source);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_turbulence_dirichlet_face_elimination") {
    ...
    std::string_view sv(k_hip_navatala_cfd_turbulence_dirichlet_face_elimination);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_two_mag_sqr_symm_grad_u") {
    ...
    std::string_view sv(k_hip_navatala_cfd_two_mag_sqr_symm_grad_u);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_attention_row_softmax") {
    ...
    std::string_view sv(k_hip_navatala_cfd_attention_row_softmax);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_attention_value_projection") {
    ...
    std::string_view sv(k_hip_navatala_cfd_attention_value_projection);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gather_mean") {
    ...
    std::string_view sv(k_hip_navatala_cfd_gather_mean);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_layer_norm_forward") {
    ...
    std::string_view sv(k_hip_navatala_cfd_layer_norm_forward);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_radius_count") {
    ...
    std::string_view sv(k_hip_navatala_cfd_radius_count);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scaled_dot_product") {
    ...
    std::string_view sv(k_hip_navatala_cfd_scaled_dot_product);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_scatter_add_atomic") {
    ...
    std::string_view sv(k_hip_navatala_cfd_scatter_add_atomic);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_spectral_complex_mul") {
    ...
    std::string_view sv(k_hip_navatala_cfd_spectral_complex_mul);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_spectral_conv1d_mode_sliced") {
    ...
    std::string_view sv(k_hip_navatala_cfd_spectral_conv1d_mode_sliced);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_spectral_conv2d_mode_sliced") {
    ...
    std::string_view sv(k_hip_navatala_cfd_spectral_conv2d_mode_sliced);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_spectral_conv3d_mode_sliced") {
    ...
    std::string_view sv(k_hip_navatala_cfd_spectral_conv3d_mode_sliced);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_spectral_conv4d_mode_sliced") {
    ...
    std::string_view sv(k_hip_navatala_cfd_spectral_conv4d_mode_sliced);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_compute_face_area_pair_weights_f32") {
    ...
    std::string_view sv(k_hip_navatala_cfd_compute_face_area_pair_weights_f32);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_restrict_by_agglomeration_f32") {
    ...
    std::string_view sv(k_hip_navatala_cfd_restrict_by_agglomeration_f32);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_restrict_by_agglomeration_gather_f32") {
    ...
    std::string_view sv(k_hip_navatala_cfd_restrict_by_agglomeration_gather_f32);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_prolongate_by_agglomeration_f32") {
    ...
    std::string_view sv(k_hip_navatala_cfd_prolongate_by_agglomeration_f32);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_diag_inv_apply_f32") {
    ...
    std::string_view sv(k_hip_navatala_cfd_diag_inv_apply_f32);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_csr_sp_m_v_mixed_f32_f64") {
    ...
    std::string_view sv(k_hip_navatala_cfd_csr_sp_m_v_mixed_f32_f64);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_chebyshev_sweep_mixed_f32_f64") {
    ...
    std::string_view sv(k_hip_navatala_cfd_chebyshev_sweep_mixed_f32_f64);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_ell_sp_m_v_mixed_f32_f64") {
    ...
    std::string_view sv(k_hip_navatala_cfd_ell_sp_m_v_mixed_f32_f64);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_chebyshev_smooth_post_fused_mixed") {
    ...
    std::string_view sv(k_hip_navatala_cfd_chebyshev_smooth_post_fused_mixed);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_proc_pack_f32") {
    ...
    std::string_view sv(k_hip_navatala_cfd_gamg_proc_pack_f32);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_proc_add_f32") {
    ...
    std::string_view sv(k_hip_navatala_cfd_gamg_proc_add_f32);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_gamg_proc_add_gather_f32") {
    ...
    std::string_view sv(k_hip_navatala_cfd_gamg_proc_add_gather_f32);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  return false;
