...
// Kernel registry partition for the `hip_cfd_vof` bucket. Compiles
...
const char* k_hip_navatala_cfd_vof_alpha_phi_all = R"kernel(
...
const char* k_hip_navatala_cfd_vof_alpha_phi_int = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_vof_alpha_phi_van_leer_int = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_vof_average_face_scalar_to_cell = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_vof_interp_scalar_face_all = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_vof_cmules_flux_diff = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_vof_cmules_flux_diff(const float* alphaPhiUn, const float* alphaPhi10, const int* faceCounts, float* outPhiCorr) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  if (((int)((int)(blockIdx.x * blockDim.x + threadIdx.x))) >= faceCounts[0]) {
    return;
  } else {
    outPhiCorr[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))] = (alphaPhiUn[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))] - alphaPhi10[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))]);
  }
}
...
extern "C" __global__ void navatala_cfd_vof_cmules_limiter_corr_prepare(const float* alpha, const float* alphaF, const int* boundaryExtremaMask, const float* phiCorr, const int* offsets, const int* faceIdx, const float* sign, const int* owner, const int* nei, const float* vol, const float* rSubDeltaT, const float* sp, const float* su, const float* psiMax, const float* psiMin, const int* counts, const float* paramsF, float* psiMaxCap, float* psiMinCap, float* sumPhip, float* mSumPhim) {
  ...
  if (((int)((int)(blockIdx.x * blockDim.x + threadIdx.x))) >= counts[0]) {
    ...
    if (boundaryDeltaExtremaCoeff < __uint_as_float(0x00000000u)) {
      boundaryDeltaExtremaCoeff = __uint_as_float(0x00000000u);
    }
    int beg = offsets[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
    int c1 = (((int)((int)(blockIdx.x * blockDim.x + threadIdx.x))) + 1);
    int end = offsets[c1];
...
const char* k_hip_navatala_cfd_vof_cmules_accumulate_correction = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_vof_cmules_accumulate_correction(float* alphaPhiBase, const float* phiCorrLimited, const int* counts, const float* paramsF) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  const int nSafeMax = (((int)(counts[1])) > 0 ? ((int)(counts[1])) - 1 : 0);
  const int safeIdx = (gid0 < nSafeMax ? gid0 : nSafeMax);
  if (gid0 >= ((int)(counts[1]))) return;
  if (((int)((int)(blockIdx.x * blockDim.x + threadIdx.x))) >= counts[1]) {
    ...
    alphaPhiBase[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))] = (alphaPhiBase[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))] + (paramsF[0] * phiCorrLimited[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))]));
  }
}

)kernel";
const char* k_hip_navatala_cfd_vof_cmules_blend_alpha = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_vof_cmules_blend_alpha(float* alpha, const float* alphaRef, const int* counts, const float* paramsF) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  const int nSafeMax = (((int)(counts[0])) > 0 ? ((int)(counts[0])) - 1 : 0);
  const int safeIdx = (gid0 < nSafeMax ? gid0 : nSafeMax);
  if (gid0 >= ((int)(counts[0]))) return;
  if (((int)((int)(blockIdx.x * blockDim.x + threadIdx.x))) >= counts[0]) {
    ...
    alpha[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))] = ((paramsF[0] * alpha[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))]) + (paramsF[1] * alphaRef[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))]));
  }
}

)kernel";
const char* k_hip_navatala_cfd_vof_cmules_update_prev_correction = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_vof_cmules_update_prev_correction(const float* alphaPhiBase, float* prevCorr, const int* counts, const float* paramsF) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  const int nSafeMax = (((int)(counts[1])) > 0 ? ((int)(counts[1])) - 1 : 0);
  const int safeIdx = (gid0 < nSafeMax ? gid0 : nSafeMax);
  if (gid0 >= ((int)(counts[1]))) return;
  if (((int)((int)(blockIdx.x * blockDim.x + threadIdx.x))) >= counts[1]) {
    ...
    prevCorr[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))] = (alphaPhiBase[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))] - prevCorr[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))]);
  }
}

)kernel";
const char* k_hip_navatala_cfd_vof_cmules_predictor_boundary_flux = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_vof_cmules_predictor_boundary_flux(const float* alphaPred, const int* boundaryFaceOwner, const int* boundaryFaceGlobal, const float* internalCoeffs, const float* boundaryCoeffs, const int* counts, float* outPhiUD) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  const int nSafeMax = (((int)(counts[3])) > 0 ? ((int)(counts[3])) - 1 : 0);
  const int safeIdx = (gid0 < nSafeMax ? gid0 : nSafeMax);
  if (gid0 >= ((int)(counts[3]))) return;
  if (((int)((int)(blockIdx.x * blockDim.x + threadIdx.x))) >= counts[3]) {
    return;
  } else {
    int ownerCell = boundaryFaceOwner[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
    int globalFace = boundaryFaceGlobal[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
    float alpha = alphaPred[ownerCell];
    float ic = internalCoeffs[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
    float bc = boundaryCoeffs[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
    outPhiUD[globalFace] = ((ic * alpha) - bc);
  }
}

)kernel";
const char* k_hip_navatala_cfd_vof_cmules_predictor_coupled_boundary_flux = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_vof_cmules_predictor_coupled_boundary_flux(const float* alphaPred, const int* boundaryFaceOwner, const int* boundaryFaceGlobal, const float* internalCoeffs, const float* boundaryCoeffs, const float* neighbourAlpha, const int* counts, float* outPhiUD) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  const int nSafeMax = (((int)(counts[0])) > 0 ? ((int)(counts[0])) - 1 : 0);
  const int safeIdx = (gid0 < nSafeMax ? gid0 : nSafeMax);
  if (gid0 >= ((int)(counts[0]))) return;
  if (((int)((int)(blockIdx.x * blockDim.x + threadIdx.x))) >= counts[3]) {
    ...
    float alphaNbr = neighbourAlpha[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
    float ic = internalCoeffs[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
    float bc = boundaryCoeffs[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
...
const char* k_hip_navatala_cfd_vof_cmules_predictor_internal_flux = R"kernel(
#include <hip/hip_runtime.h>
extern "C" __global__ void navatala_cfd_vof_cmules_predictor_internal_flux(const float* alphaPred, const float* phiCN, const int* owner, const int* nei, const int* counts, float* outPhiUD) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  ...
  if (((int)((int)(blockIdx.x * blockDim.x + threadIdx.x))) >= counts[2]) {
    ...
    float ph = phiCN[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
    int o = owner[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
    int n = nei[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))];
    float a = __uint_as_float(0x00000000u);
    if (ph >= __uint_as_float(0x00000000u)) {
    ...
    outPhiUD[((int)((int)(blockIdx.x * blockDim.x + threadIdx.x)))] = (ph * a);
  }
}
...
const char* k_hip_navatala_cfd_vof_mules_cell_lambda = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_vof_mules_cell_sums = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_vof_mules_face_update = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_vof_mules_fill_lambda = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_vof_mules_init = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_vof_phi_b_d_corr = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_vof_phir = R"kernel(
#include <hip/hip_runtime.h>
...
const char* k_hip_navatala_cfd_vof_rho_phi_accumulate = R"kernel(
#include <hip/hip_runtime.h>
...
const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_alpha_phi_all[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_alpha_phi_all = {
  1,
  "navatala_cfd_vof_alpha_phi_all",
  "hip",
  "navatala_cfd_vof_alpha_phi_all",
  "kernel:hip:navatala_cfd_vof_alpha_phi_all",
  "abi-r1:hip:navatala_cfd_vof_alpha_phi_all",
  "abi-r1:hip:navatala_cfd_vof_alpha_phi_all",
  5,
  kAbiArgs_hip_navatala_cfd_vof_alpha_phi_all
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_alpha_phi_int[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_alpha_phi_int = {
  1,
  "navatala_cfd_vof_alpha_phi_int",
  "hip",
  "navatala_cfd_vof_alpha_phi_int",
  "kernel:hip:navatala_cfd_vof_alpha_phi_int",
  "abi-r1:hip:navatala_cfd_vof_alpha_phi_int",
  "abi-r1:hip:navatala_cfd_vof_alpha_phi_int",
  8,
  kAbiArgs_hip_navatala_cfd_vof_alpha_phi_int
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_alpha_phi_van_leer_int[] = {
  { "alpha", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "phi", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "owner", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "nei", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_alpha_phi_van_leer_int = {
  1,
  "navatala_cfd_vof_alpha_phi_van_leer_int",
  "hip",
  "navatala_cfd_vof_alpha_phi_van_leer_int",
  "kernel:hip:navatala_cfd_vof_alpha_phi_van_leer_int",
  "abi-r1:hip:navatala_cfd_vof_alpha_phi_van_leer_int",
  "abi-r1:hip:navatala_cfd_vof_alpha_phi_van_leer_int",
  15,
  kAbiArgs_hip_navatala_cfd_vof_alpha_phi_van_leer_int
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_average_face_scalar_to_cell[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_average_face_scalar_to_cell = {
  1,
  "navatala_cfd_vof_average_face_scalar_to_cell",
  "hip",
  "navatala_cfd_vof_average_face_scalar_to_cell",
  "kernel:hip:navatala_cfd_vof_average_face_scalar_to_cell",
  "abi-r1:hip:navatala_cfd_vof_average_face_scalar_to_cell",
  "abi-r1:hip:navatala_cfd_vof_average_face_scalar_to_cell",
  7,
  kAbiArgs_hip_navatala_cfd_vof_average_face_scalar_to_cell
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_interp_scalar_face_all[] = {
  { "alpha", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_interp_scalar_face_all = {
  1,
  "navatala_cfd_vof_interp_scalar_face_all",
  "hip",
  "navatala_cfd_vof_interp_scalar_face_all",
  "kernel:hip:navatala_cfd_vof_interp_scalar_face_all",
  "abi-r1:hip:navatala_cfd_vof_interp_scalar_face_all",
  "abi-r1:hip:navatala_cfd_vof_interp_scalar_face_all",
  7,
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_alpha_update = {
  1,
  "navatala_cfd_vof_alpha_update",
  "hip",
  "navatala_cfd_vof_alpha_update",
  "kernel:hip:navatala_cfd_vof_alpha_update",
...
const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_cmules_flux_diff[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_cmules_flux_diff = {
  1,
  "navatala_cfd_vof_cmules_flux_diff",
  "hip",
  "navatala_cfd_vof_cmules_flux_diff",
  "kernel:hip:navatala_cfd_vof_cmules_flux_diff",
  "abi-r1:hip:navatala_cfd_vof_cmules_flux_diff",
  "abi-r1:hip:navatala_cfd_vof_cmules_flux_diff",
  4,
  kAbiArgs_hip_navatala_cfd_vof_cmules_flux_diff
};
...
const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_cmules_accumulate_correction[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_cmules_accumulate_correction = {
  1,
  "navatala_cfd_vof_cmules_accumulate_correction",
  "hip",
  "navatala_cfd_vof_cmules_accumulate_correction",
  "kernel:hip:navatala_cfd_vof_cmules_accumulate_correction",
  "abi-r1:hip:navatala_cfd_vof_cmules_accumulate_correction",
  "abi-r1:hip:navatala_cfd_vof_cmules_accumulate_correction",
  4,
  kAbiArgs_hip_navatala_cfd_vof_cmules_accumulate_correction
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_cmules_blend_alpha[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_cmules_blend_alpha = {
  1,
  "navatala_cfd_vof_cmules_blend_alpha",
  "hip",
  "navatala_cfd_vof_cmules_blend_alpha",
  "kernel:hip:navatala_cfd_vof_cmules_blend_alpha",
  "abi-r1:hip:navatala_cfd_vof_cmules_blend_alpha",
  "abi-r1:hip:navatala_cfd_vof_cmules_blend_alpha",
  4,
  kAbiArgs_hip_navatala_cfd_vof_cmules_blend_alpha
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_cmules_update_prev_correction[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_cmules_update_prev_correction = {
  1,
  "navatala_cfd_vof_cmules_update_prev_correction",
  "hip",
  "navatala_cfd_vof_cmules_update_prev_correction",
  "kernel:hip:navatala_cfd_vof_cmules_update_prev_correction",
  "abi-r1:hip:navatala_cfd_vof_cmules_update_prev_correction",
  "abi-r1:hip:navatala_cfd_vof_cmules_update_prev_correction",
  4,
  kAbiArgs_hip_navatala_cfd_vof_cmules_update_prev_correction
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_cmules_predictor_boundary_flux[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_cmules_predictor_boundary_flux = {
  1,
  "navatala_cfd_vof_cmules_predictor_boundary_flux",
  "hip",
  "navatala_cfd_vof_cmules_predictor_boundary_flux",
  "kernel:hip:navatala_cfd_vof_cmules_predictor_boundary_flux",
  "abi-r1:hip:navatala_cfd_vof_cmules_predictor_boundary_flux",
  "abi-r1:hip:navatala_cfd_vof_cmules_predictor_boundary_flux",
  7,
  kAbiArgs_hip_navatala_cfd_vof_cmules_predictor_boundary_flux
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_cmules_predictor_coupled_boundary_flux[] = {
  { "alphaPred", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "boundaryFaceOwner", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "boundaryFaceGlobal", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "internalCoeffs", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_cmules_predictor_coupled_boundary_flux = {
  1,
  "navatala_cfd_vof_cmules_predictor_coupled_boundary_flux",
  "hip",
  "navatala_cfd_vof_cmules_predictor_coupled_boundary_flux",
  "kernel:hip:navatala_cfd_vof_cmules_predictor_coupled_boundary_flux",
  "abi-r1:hip:navatala_cfd_vof_cmules_predictor_coupled_boundary_flux",
  "abi-r1:hip:navatala_cfd_vof_cmules_predictor_coupled_boundary_flux",
  8,
  kAbiArgs_hip_navatala_cfd_vof_cmules_predictor_coupled_boundary_flux
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_cmules_predictor_internal_flux[] = {
  { "alphaPred", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_cmules_predictor_internal_flux = {
  1,
  "navatala_cfd_vof_cmules_predictor_internal_flux",
  "hip",
  "navatala_cfd_vof_cmules_predictor_internal_flux",
  "kernel:hip:navatala_cfd_vof_cmules_predictor_internal_flux",
  "abi-r1:hip:navatala_cfd_vof_cmules_predictor_internal_flux",
  "abi-r1:hip:navatala_cfd_vof_cmules_predictor_internal_flux",
  6,
  kAbiArgs_hip_navatala_cfd_vof_cmules_predictor_internal_flux
};
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_mules_apply = {
  1,
  "navatala_cfd_vof_mules_apply",
  "hip",
  "navatala_cfd_vof_mules_apply",
  "kernel:hip:navatala_cfd_vof_mules_apply",
  "abi-r1:hip:navatala_cfd_vof_mules_apply",
  "abi-r1:hip:navatala_cfd_vof_mules_apply",
  6,
  kAbiArgs_hip_navatala_cfd_vof_mules_apply
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_mules_cell_lambda[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_mules_cell_lambda = {
  1,
  "navatala_cfd_vof_mules_cell_lambda",
  "hip",
  "navatala_cfd_vof_mules_cell_lambda",
  "kernel:hip:navatala_cfd_vof_mules_cell_lambda",
  "abi-r1:hip:navatala_cfd_vof_mules_cell_lambda",
  "abi-r1:hip:navatala_cfd_vof_mules_cell_lambda",
  10,
  kAbiArgs_hip_navatala_cfd_vof_mules_cell_lambda
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_mules_cell_sums[] = {
  { "phiCorr", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "lambda", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "offsets", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "faceIdx", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_mules_cell_sums = {
  1,
  "navatala_cfd_vof_mules_cell_sums",
  "hip",
  "navatala_cfd_vof_mules_cell_sums",
  "kernel:hip:navatala_cfd_vof_mules_cell_sums",
  "abi-r1:hip:navatala_cfd_vof_mules_cell_sums",
  "abi-r1:hip:navatala_cfd_vof_mules_cell_sums",
  9,
  kAbiArgs_hip_navatala_cfd_vof_mules_cell_sums
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_mules_face_update[] = {
  { "phiCorr", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_mules_face_update = {
  1,
  "navatala_cfd_vof_mules_face_update",
  "hip",
  "navatala_cfd_vof_mules_face_update",
  "kernel:hip:navatala_cfd_vof_mules_face_update",
  "abi-r1:hip:navatala_cfd_vof_mules_face_update",
  "abi-r1:hip:navatala_cfd_vof_mules_face_update",
  8,
  kAbiArgs_hip_navatala_cfd_vof_mules_face_update
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_mules_fill_lambda[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_mules_fill_lambda = {
  1,
  "navatala_cfd_vof_mules_fill_lambda",
  "hip",
  "navatala_cfd_vof_mules_fill_lambda",
  "kernel:hip:navatala_cfd_vof_mules_fill_lambda",
  "abi-r1:hip:navatala_cfd_vof_mules_fill_lambda",
  "abi-r1:hip:navatala_cfd_vof_mules_fill_lambda",
  3,
  kAbiArgs_hip_navatala_cfd_vof_mules_fill_lambda
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_mules_init[] = {
  { "alpha", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "alphaF", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_mules_init = {
  1,
  "navatala_cfd_vof_mules_init",
  "hip",
  "navatala_cfd_vof_mules_init",
  "kernel:hip:navatala_cfd_vof_mules_init",
  "abi-r1:hip:navatala_cfd_vof_mules_init",
  "abi-r1:hip:navatala_cfd_vof_mules_init",
  17,
  kAbiArgs_hip_navatala_cfd_vof_mules_init
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_phi_b_d_corr[] = {
  { "alpha", 0, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "phiAll", 1, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "owner", 2, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
  { "nei", 3, KernelArgumentRole::Input, KernelAccessMode::ReadOnly, GpuRuntime::MemoryKind::Device, true, 0, 0, 256, nullptr, 0, 0 },
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_phi_b_d_corr = {
  1,
  "navatala_cfd_vof_phi_b_d_corr",
  "hip",
  "navatala_cfd_vof_phi_b_d_corr",
  "kernel:hip:navatala_cfd_vof_phi_b_d_corr",
  "abi-r1:hip:navatala_cfd_vof_phi_b_d_corr",
  "abi-r1:hip:navatala_cfd_vof_phi_b_d_corr",
  9,
  kAbiArgs_hip_navatala_cfd_vof_phi_b_d_corr
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_phir[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_phir = {
  1,
  "navatala_cfd_vof_phir",
  "hip",
  "navatala_cfd_vof_phir",
  "kernel:hip:navatala_cfd_vof_phir",
  "abi-r1:hip:navatala_cfd_vof_phir",
  "abi-r1:hip:navatala_cfd_vof_phir",
  13,
  kAbiArgs_hip_navatala_cfd_vof_phir
};

const KernelArgumentInfo kAbiArgs_hip_navatala_cfd_vof_rho_phi_accumulate[] = {
...
const KernelAbiManifestInfo kAbiManifest_hip_navatala_cfd_vof_rho_phi_accumulate = {
  1,
  "navatala_cfd_vof_rho_phi_accumulate",
  "hip",
  "navatala_cfd_vof_rho_phi_accumulate",
  "kernel:hip:navatala_cfd_vof_rho_phi_accumulate",
  "abi-r1:hip:navatala_cfd_vof_rho_phi_accumulate",
  "abi-r1:hip:navatala_cfd_vof_rho_phi_accumulate",
  5,
  kAbiArgs_hip_navatala_cfd_vof_rho_phi_accumulate
};

bool tryGetKernelAbiManifest_hip_cfd_vof(const std::string& backend, const std::string& kernelName, const KernelAbiManifestInfo*& out) {
  if (backend == "hip" && kernelName == "navatala_cfd_vof_alpha_phi_all") {
    out = &kAbiManifest_hip_navatala_cfd_vof_alpha_phi_all;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_alpha_phi_int") {
    out = &kAbiManifest_hip_navatala_cfd_vof_alpha_phi_int;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_alpha_phi_van_leer_int") {
    out = &kAbiManifest_hip_navatala_cfd_vof_alpha_phi_van_leer_int;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_average_face_scalar_to_cell") {
    out = &kAbiManifest_hip_navatala_cfd_vof_average_face_scalar_to_cell;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_interp_scalar_face_all") {
  ...
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_flux_diff") {
    out = &kAbiManifest_hip_navatala_cfd_vof_cmules_flux_diff;
    return true;
  }
  ...
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_accumulate_correction") {
    out = &kAbiManifest_hip_navatala_cfd_vof_cmules_accumulate_correction;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_blend_alpha") {
    out = &kAbiManifest_hip_navatala_cfd_vof_cmules_blend_alpha;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_update_prev_correction") {
    out = &kAbiManifest_hip_navatala_cfd_vof_cmules_update_prev_correction;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_predictor_boundary_flux") {
    out = &kAbiManifest_hip_navatala_cfd_vof_cmules_predictor_boundary_flux;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_predictor_coupled_boundary_flux") {
    out = &kAbiManifest_hip_navatala_cfd_vof_cmules_predictor_coupled_boundary_flux;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_predictor_internal_flux") {
    out = &kAbiManifest_hip_navatala_cfd_vof_cmules_predictor_internal_flux;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_mules_apply") {
    out = &kAbiManifest_hip_navatala_cfd_vof_mules_apply;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_mules_cell_lambda") {
    out = &kAbiManifest_hip_navatala_cfd_vof_mules_cell_lambda;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_mules_cell_sums") {
    out = &kAbiManifest_hip_navatala_cfd_vof_mules_cell_sums;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_mules_face_update") {
    out = &kAbiManifest_hip_navatala_cfd_vof_mules_face_update;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_mules_fill_lambda") {
    out = &kAbiManifest_hip_navatala_cfd_vof_mules_fill_lambda;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_mules_init") {
    out = &kAbiManifest_hip_navatala_cfd_vof_mules_init;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_phi_b_d_corr") {
    out = &kAbiManifest_hip_navatala_cfd_vof_phi_b_d_corr;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_phir") {
    out = &kAbiManifest_hip_navatala_cfd_vof_phir;
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_rho_phi_accumulate") {
    out = &kAbiManifest_hip_navatala_cfd_vof_rho_phi_accumulate;
    return true;
  }
  out = nullptr;
  return false;
...
bool tryGetKernelSource_hip_cfd_vof(const std::string& backend, const std::string& kernelName, GpuRuntime::ProgramSource& out) {
  if (backend == "hip" && kernelName == "navatala_cfd_vof_alpha_phi_all") {
    out.kind = GpuRuntime::ProgramSource::Kind::HipCpp;
    out.entryPoint = "navatala_cfd_vof_alpha_phi_all";
    std::string_view sv(k_hip_navatala_cfd_vof_alpha_phi_all);
  ...
  if (backend == "hip" && kernelName == "navatala_cfd_vof_alpha_phi_int") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_alpha_phi_int);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_alpha_phi_van_leer_int") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_alpha_phi_van_leer_int);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_average_face_scalar_to_cell") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_average_face_scalar_to_cell);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_interp_scalar_face_all") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_interp_scalar_face_all);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_alpha_face_average_internal") {
  ...
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_flux_diff") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_cmules_flux_diff);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_limiter_corr_prepare") {
  ...
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_accumulate_correction") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_cmules_accumulate_correction);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_blend_alpha") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_cmules_blend_alpha);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_update_prev_correction") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_cmules_update_prev_correction);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_predictor_boundary_flux") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_cmules_predictor_boundary_flux);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_predictor_coupled_boundary_flux") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_cmules_predictor_coupled_boundary_flux);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_cmules_predictor_internal_flux") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_cmules_predictor_internal_flux);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_mules_apply") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_mules_apply);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_mules_cell_lambda") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_mules_cell_lambda);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_mules_cell_sums") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_mules_cell_sums);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_mules_face_update") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_mules_face_update);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_mules_fill_lambda") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_mules_fill_lambda);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_mules_init") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_mules_init);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_phi_b_d_corr") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_phi_b_d_corr);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_phir") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_phir);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  if (backend == "hip" && kernelName == "navatala_cfd_vof_rho_phi_accumulate") {
    ...
    std::string_view sv(k_hip_navatala_cfd_vof_rho_phi_accumulate);
    out.bytes.assign(sv.begin(), sv.end());
    return true;
  }
  return false;
