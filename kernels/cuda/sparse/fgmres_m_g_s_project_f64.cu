...
extern "C" __global__ void navatala_sparse_fgmres_m_g_s_project_f64(const double* vi, const unsigned int* n, double* w, double* hij) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int gid = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int lid = (int)(threadIdx.x);
  int N = ((int)(n[0]));
  bool inBounds = (gid < N);
  double prod = ((inBounds) ? ((w[gid] * vi[gid])) : (__longlong_as_double(0x0000000000000000ull)));
