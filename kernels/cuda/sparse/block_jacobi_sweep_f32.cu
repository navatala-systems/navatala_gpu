...
extern "C" __global__ void navatala_sparse_block_jacobi_sweep_f32(const unsigned int* rowPtr, const unsigned int* colIdx, const float* values, const float* diagBlocks, const float* b, const float* x, const float* omega, const unsigned int* nBlockRows, const unsigned int* blockDim, float* xNew) {
  ...
  if (blockRow < N) {
    ...
    if (bdm == 4) {
      ...
      float offDiag2 = __uint_as_float(0x00000000u);
      float offDiag3 = __uint_as_float(0x00000000u);
      ...
      float oneMinusOm = (__uint_as_float(0x3f800000u) - om);
      ...
      for (int eq = 0; eq < (int)(bdm); ++eq) {
        ...
        float xr = x[grow];
        xNew[grow] = (((__uint_as_float(0x3f800000u) - om) * xr) + (om * (invVal * rhs)));
      }
    }
  }
}
