...
extern "C" __global__ void navatala_sparse_ldu_to_csr_fill_f64(const unsigned int* owner, const unsigned int* neighbour, const double* upper, const double* lower, const unsigned int* rowPtr, const unsigned int* nInternalFaces, const unsigned int* nCells, unsigned int* colInd, double* values, unsigned int* insertIdx) {
  ...
  if ((face < nFaces)) {
    ...
    unsigned int posNbr = atomicAdd(&(insertIdx[nbr]), 1u);
