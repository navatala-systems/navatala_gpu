...
extern "C" __global__ void navatala_sparse_idr_shadow_project_f32(const float* P, const float* r, const unsigned int* n, const unsigned int* s, float* c) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int gid = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int lid = (int)(threadIdx.x);
  int k = (int)(blockIdx.x);
  int N = ((int)(n[0]));
  float val = (((gid < N)) ? ((P[(gid + (k * N))] * r[gid])) : (__uint_as_float(0x00000000u)));
