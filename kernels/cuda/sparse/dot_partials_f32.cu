...
extern "C" __global__ void navatala_sparse_dot_partials_f32(const float* x, const float* y, const unsigned int* n, float* partials) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int gid = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int lid = (int)(threadIdx.x);
  int N = ((int)(n[0]));
  float prod = (((gid < N)) ? ((x[gid] * y[gid])) : (__uint_as_float(0x00000000u)));
