...
extern "C" __global__ void navatala_sparse_nrm2_partials_f32(const float* x, const unsigned int* n, float* partials) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int gid = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int lid = (int)(threadIdx.x);
  int N = ((int)(n[0]));
  float sq = (((gid < N)) ? ((x[gid] * x[gid])) : (__uint_as_float(0x00000000u)));
