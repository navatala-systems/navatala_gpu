...
extern "C" __global__ void navatala_sparse_sparse_jaccard_distance_f32(const float* a, const float* b, const unsigned int* nnz, float* result) {
  ...
  for (int loopIdx = 0; loopIdx < (int)(nnzVal); ++loopIdx) {
    ...
    if (currentIdx < nnzVal) {
      ...
      bool bNonZero = (bVal != __uint_as_float(0x00000000u));
  ...
  if (lidU32 == 0u) {
    ...
    float jaccardSim = ((unionIsZero) ? (__uint_as_float(0x00000000u)) : ((intersectF / unionF)));
    float jaccardDist = (__uint_as_float(0x3f800000u) - jaccardSim);
    float finalResult = ((unionIsZero) ? (__uint_as_float(0x00000000u)) : (jaccardDist));
