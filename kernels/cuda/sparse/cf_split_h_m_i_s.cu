...
extern "C" __global__ void navatala_sparse_cf_split_h_m_i_s(const unsigned int* rowPtr, const unsigned int* colIdx, const unsigned int* strongMask, const unsigned int* nRows, const unsigned int* seed, int* cfMarking) {
  ...
  if (row < N) {
    ...
    unsigned int boostedPri = (priority + deg);
