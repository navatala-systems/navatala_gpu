...
extern "C" __global__ void navatala_sparse_sparse_l2_distance_f64(const double* a, const double* b, const unsigned int* nnz, double* result) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  __shared__ double sdata[256];
  unsigned int gid = ((unsigned int)((int)(blockIdx.x * blockDim.x + threadIdx.x)));
  unsigned int lidU32 = ((unsigned int)((int)(threadIdx.x)));
  unsigned int nnzVal = nnz[0u];
  double localSumSqAccum = __longlong_as_double(0x0000000000000000ull);
