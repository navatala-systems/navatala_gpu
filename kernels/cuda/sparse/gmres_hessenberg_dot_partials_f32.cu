...
extern "C" __global__ void navatala_sparse_gmres_hessenberg_dot_partials_f32(const float* V, const float* w, const unsigned int* n, const unsigned int* j, float* partials) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int gid = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int lid = (int)(threadIdx.x);
  int N = ((int)(n[0]));
  float sq = (((gid < N)) ? ((w[gid] * w[gid])) : (__uint_as_float(0x00000000u)));
