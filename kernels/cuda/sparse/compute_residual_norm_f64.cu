...
extern "C" __global__ void navatala_sparse_compute_residual_norm_f64(const double* residual, const unsigned int* n, double* normSq) {
  ...
  double ri = ((inBounds) ? (residual[i]) : (__longlong_as_double(0x0000000000000000ull)));
  double riSq = (ri * ri);
  sdata[lid] = riSq;
  __syncthreads();
