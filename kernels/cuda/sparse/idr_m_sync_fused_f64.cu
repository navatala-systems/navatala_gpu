...
extern "C" __global__ void navatala_sparse_idr_m_sync_fused_f64(const double* P, const double* r, const double* Minv, const unsigned int* n, const unsigned int* s, double* c) {
  ...
  for (int j = 0; j < (int)(S); ++j) {
    double pjr = (((gid < N)) ? ((P[(gid + (j * N))] * r[gid])) : (__longlong_as_double(0x0000000000000000ull)));
