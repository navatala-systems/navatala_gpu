...
extern "C" __global__ void navatala_sparse_cg_residual_norm_partials_f64(const double* r, const unsigned int* n, double* partials) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int gid = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int lid = (int)(threadIdx.x);
  int N = ((int)(n[0]));
  double sq = (((gid < N)) ? ((r[gid] * r[gid])) : (__longlong_as_double(0x0000000000000000ull)));
