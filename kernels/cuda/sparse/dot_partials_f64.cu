...
extern "C" __global__ void navatala_sparse_dot_partials_f64(const double* x, const double* y, const unsigned int* n, double* partials) {
  int gid0 = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int gid = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  int lid = (int)(threadIdx.x);
  int N = ((int)(n[0]));
  double prod = (((gid < N)) ? ((x[gid] * y[gid])) : (__longlong_as_double(0x0000000000000000ull)));
