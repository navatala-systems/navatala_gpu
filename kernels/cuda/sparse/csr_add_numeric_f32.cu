...
extern "C" __global__ void navatala_sparse_csr_add_numeric_f32(const unsigned int* ArowPtr, const unsigned int* AcolIdx, const float* Avalues, const unsigned int* BrowPtr, const unsigned int* BcolIdx, const float* Bvalues, const unsigned int* CrowPtr, const float* alpha, const float* beta, const unsigned int* nRows, unsigned int* CcolIdx, float* Cvalues) {
    ...
    for (int __iter = 0; __iter < 256; ++__iter) {
      ...
      if ((acol == bcol)) {
        ...
        if ((acol < bcol)) {
          ...
          int tgt3 = (crs + ci);
          CcolIdx[tgt3] = ((unsigned int)(bcol));
