...
extern "C" __global__ void navatala_sparse_csr_sort_after_ldu_fill_f64(const unsigned int* rowPtr, const unsigned int* nRows, unsigned int* colInd, double* values) {
  ...
  if (row < N) {
    ...
    for (int i = 0; i < (int)(count); ++i) {
      int pos = (start + i);
      unsigned int keyCol = colInd[pos];
