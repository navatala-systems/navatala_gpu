...
extern "C" __global__ void navatala_sparse_chebyshev_smooth_f32(const unsigned int* rowPtr, const unsigned int* colInd, const float* values, const float* x, const float* xPrev, const float* diagInv, const float* b, const unsigned int* n, const float* alpha, const float* beta, float* xNext) {
