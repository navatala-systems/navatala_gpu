...
#include <cuda_bf16.h>
extern "C" __global__ void navatala_ml_avgpool_include_pad_bf16(const __nv_bfloat16* _input, const unsigned int* window, const unsigned int* stride, const unsigned int* inCount, const unsigned int* outCount, __nv_bfloat16* _output) {
  ...
  if ((gid < outCountVal)) {
    ...
    _output[gid] = ((__nv_bfloat16)((sum / ((float)(windowVal)))));
  }
}
