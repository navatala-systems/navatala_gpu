...
#include <cuda_bf16.h>
extern "C" __global__ void navatala_ml_spatial_batchnorm_bf16(const __nv_bfloat16* x, const __nv_bfloat16* gamma, const __nv_bfloat16* beta, const unsigned int* N, const unsigned int* C, const unsigned int* HW, const float* eps, __nv_bfloat16* _output) {
  ...
  unsigned int CHW = (Cv * HWv);
  unsigned int chBase = (grp * HWv);
  unsigned int countVal = (Nv * HWv);
  ...
  for (int itW = 0; itW < (int)(numIters); ++itW) {
    if (((lid + (((unsigned int)(itW)) * 256u)) < countVal)) {
      ...
      _output[(((((lid + (((unsigned int)(itW)) * 256u)) / HWv) * CHW) + chBase) + ((lid + (((unsigned int)(itW)) * 256u)) % HWv))] = ((__nv_bfloat16)(((gv * xn) + bv)));
    }
  }
}
