...
#include <cuda_bf16.h>
extern "C" __global__ void navatala_ml_instancenorm_bf16(const __nv_bfloat16* x, const __nv_bfloat16* gamma, const __nv_bfloat16* beta, const unsigned int* C, const unsigned int* HW, const float* eps, __nv_bfloat16* _output) {
  ...
  for (int itW = 0; itW < (int)(numIters); ++itW) {
    if (((lid + (((unsigned int)(itW)) * 256u)) < countVal)) {
      ...
      _output[(base + (lid + (((unsigned int)(itW)) * 256u)))] = ((__nv_bfloat16)(((gv * xn) + bv)));
    }
  }
}
