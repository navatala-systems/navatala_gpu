...
#include <cuda_bf16.h>
extern "C" __global__ void navatala_ml_bernoulli_bf16(const unsigned int* count, const unsigned int* seed, const float* p, __nv_bfloat16* _output) {
  ...
  if ((gid < countVal)) {
    ...
    __nv_bfloat16 outV = ((__nv_bfloat16)(bit));
    _output[gid] = outV;
  }
}
